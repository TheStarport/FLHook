...
void LoadSettings()
{
	...
	std::string scPluginCfgFile = std::string(szCurDir) + "\\flhook_plugins\\rename.cfg";

	set_bEnableRenameMe = IniGetB(scPluginCfgFile, "Rename", "EnableRename", false);
	set_bEnableMoveChar = IniGetB(scPluginCfgFile, "Rename", "EnableMoveChar", false);
...
bool CreateNewCharacter(struct SCreateCharacterInfo const& si, uint& iClientID)
{
	if (set_bCharnameTags)
	{
		...
		for (std::map<std::wstring, TAG_DATA>::iterator i = mapTagToPassword.begin(); i != mapTagToPassword.end(); ++i)
		{
			if (wscCharname.find(i->second.tag) == 0 && i->second.rename_password.size() != 0)
			{
...
// it.
void CharacterSelect_AFTER(std::string& szCharFilename, uint& iClientID)
{
	if (set_bCharnameTags)
	{
	...
	if (set_bCharnameTags)
	{
		...
		for (std::map<std::wstring, TAG_DATA>::iterator i = mapTagToPassword.begin(); i != mapTagToPassword.end(); ++i)
		{
			if (tag == i->second.tag && master_password == i->second.master_password)
			{
				...
				SaveSettings();
				PrintUserCmdText(
				    iClientID, L"OK Created rename password %s for tag %s", rename_password.c_str(), tag.c_str());
				return;
			}
		}
		PrintUserCmdText(iClientID, L"ERR tag or master password are invalid");
...
void Timer()
{
	...
	while (pendingRenames.size())
	{
		...
		try
		{
			...
			if (!::MoveFileExA(
			        o.scSourceFile.c_str(), o.scDestFileTemp.c_str(),
			...
			// Move files around
			if (!::MoveFileExA(
			        o.scDestFileTemp.c_str(), o.scDestFile.c_str(), MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH))
				throw "move failed";
...
void UserCmd_RenameMe(uint iClientID, const std::wstring& wscParam)
{
	...
	if (set_bCharnameTags)
	{
		...
		for (std::map<std::wstring, TAG_DATA>::iterator i = mapTagToPassword.begin(); i != mapTagToPassword.end(); ++i)
		{
			if (wscNewCharname.find(i->first) == 0 && i->second.rename_password.size() != 0)
			{
				...
				else if (wscPassword != i->second.master_password && wscPassword != i->second.rename_password)
				{
					PrintUserCmdText(
					    iClientID,
					    L"ERR Name starts with an owned tag. "
...
void AdminCmd_ShowTags(CCmds* cmds)
{
	...
	for (std::map<std::wstring, TAG_DATA>::iterator i = mapTagToPassword.begin(); i != mapTagToPassword.end(); ++i)
	{
		...
		cmds->Print(
		    ...
		    i->second.tag.c_str(), i->second.master_password.c_str(), i->second.rename_password.c_str(), days,
		    i->second.description.c_str());
	}
	cmds->Print(L"OK");
}

void AdminCmd_AddTag(
    CCmds* cmds, const std::wstring& tag, const std::wstring& password, const std::wstring& description)
{
	if (!(cmds->rights & RIGHT_SUPERADMIN))
	{
		cmds->Print(L"ERR No permission");
