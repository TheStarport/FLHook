      ...
      for (auto &ping : ConData[iClientID].lstPing) {
        ...
        if (iLastPing != 0) {
          ConData[iClientID].iPingFluctuation +=
              (uint)sqrt((double)pow(((float)ping - (float)iLastPing), 2));
        }
        iLastPing = ping;
      }
...
void TimerUpdateLossData() {
  ...
  while (pPD = Players.traverse_active(pPD)) {
    ...
    iNewSent = (ci.dwPacketsSentGuaranteed + ci.dwPacketsSentNonGuaranteed) -
...
EXPORT int __stdcall Update() {
  ...
  return 0; // it doesnt matter what we return here since we have set the return
            // code to "DEFAULT_RETURNCODE", so FLHook will just ignore it
}
...
EXPORT void __stdcall SPObjUpdate(struct SSPObjUpdateInfo const &ui,
  ...
  if (set_iLagDetectionFrame && ConData[iClientID].tmLastObjUpdate &&
    ...
    int iDiff =
        (int)sqrt(pow((long double)((int)iTimeDiff - (int)iTimestampDiff), 2));
    iDiff -= g_iServerLoad;
    if (iDiff < 0)
      iDiff = 0;
