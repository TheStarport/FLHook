...
void HkTimerCheckKick() {
  ...
  for (map<uint, CLOAK_INFO>::iterator ci = mapClientsCloak.begin();
    ...
    if (iShipID && info.bCanCloak) {
      switch (info.iState) {
      case STATE_CLOAK_OFF:
        // Send cloak state for uncloaked cloak-able players (only for them in
        // space) this is the code to fix the bug where players wouldnt always
        // see uncloaked players
        XActivateEquip ActivateEq;
  ...
  if (obj) {
    ...
    if (cship) {
      if (mapClientsCloak[iClientID].arch.iHoldSizeLimit != 0 &&
        ...
        PrintUserCmdText(
            iClientID, L"Cloaking device will not function on this ship type");
        mapClientsCloak[iClientID].iState = STATE_CLOAK_INVALID;
        SetState(iClientID, iShip, STATE_CLOAK_OFF);
        return true;
      }
