...
void LoadSettings()
{
	...
	std::string scPluginCfgFile = std::string(szCurDir) + "\\flhook_plugins\\cargo_drop.cfg";
	...
	set_iLootCrateID =
	    CreateID(IniGetS(scPluginCfgFile, "General", "CargoDropContainer", "lootcrate_ast_loot_metal").c_str());
	set_iHullItem1ID =
	    CreateID(IniGetS(scPluginCfgFile, "General", "HullDrop1NickName", "commodity_super_alloys").c_str());
	set_iHullItem2ID =
	    CreateID(IniGetS(scPluginCfgFile, "General", "HullDrop2NickName", "commodity_engine_components").c_str());
	set_wscDisconnectInSpaceMsg =
	    stows(IniGetS(scPluginCfgFile, "General", "DisconnectMsg", "%player is attempting to engage cloaking device"));
					...
					if (HkEnumCargo(wscCharname, lstCargo, iRemainingHoldSize) == HKE_OK)
					{
						for (auto& item : lstCargo)
						{
							if (!item.bMounted && set_mapNoLootItems.find(item.iArchID) == set_mapNoLootItems.end())
							{
								...
								Server.MineAsteroid(
								    iSystem, vLoc, set_iLootCrateID, item.iArchID, item.iCount, iClientID);
							}
						}
					}
...
void SendDeathMsg(const std::wstring& wscMsg, uint& iSystem, uint& iClientIDVictim, uint& iClientIDKiller)
{
	...
	if (HkEnumCargo((const wchar_t*)Players.GetActiveCharacterName(iClientIDVictim), lstCargo, iRemainingHoldSize) !=
	    HKE_OK)
		return;
	...
	if (iHullDrop > 0)
	{
		...
		Server.MineAsteroid(
		    iSystem, myLocation, set_iLootCrateID, set_iHullItem2ID, (int)(0.5 * (float)iHullDrop), iClientIDVictim);
	}
}
