...
void LoadSettings()
{
	...
	std::string configFile = std::string(szCurDir) + "\\flhook_plugins\\deathpenalty.cfg";
			...
			if (ini.is_header("ExcludedSystems"))
				while (ini.read_value())
				{
					if (ini.is_value("system"))
						ExcludedSystems.push_back(CreateID(ini.get_value_string(0)));
				}
			...
			if (ini.is_header("ShipOverrides"))
				while (ini.read_value())
				{
					if (ini.is_value("ship"))
						FractionOverridesbyShip[CreateID(ini.get_value_string(0))] = ini.get_value_float(1);
				}
		}
		ini.close();
	}
...
bool bExcludedSystem(uint iClientID)
{
	...
	return (std::find(ExcludedSystems.begin(), ExcludedSystems.end(), iSystemID) != ExcludedSystems.end());
}
...
float fShipFractionOverride(uint iClientID)
{
	...
	if (FractionOverridesbyShip.find(iShipArchID) != FractionOverridesbyShip.end())
	...
	if (iClientID != -1 && !bExcludedSystem(iClientID))
	{
		...
		if (iKillerID && set_fDeathPenaltyKiller)
		{
			...
			if (iGive)
			{
				...
				PrintUserCmdText(
				    iKillerID, L"Death penalty: given " + ToMoneyStr(iGive) + L" credits from %s's death penalty.",
...
void UserCmd_DP(uint iClientID, const std::wstring& wscParam)
{
	...
	else
	{
		...
		if (!bExcludedSystem(iClientID))
		{
			float fValue;
			pub::Player::GetAssetValue(iClientID, fValue);
			int iOwed = static_cast<int>(fValue * fShipFractionOverride(set_fDeathPenalty));
