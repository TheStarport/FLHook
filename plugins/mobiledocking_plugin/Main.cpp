...
bool UserCmd_Process(uint client, const std::wstring &wscCmd) {
  ...
  if (wscCmd.find(L"/listdocked") == 0) {
    ...
    if (iDockedClientID) {
      ...
      if (!ship) {
        JumpToLocation(iDockedClientID, clients[iDockedClientID].iCarrierSystem,
  ...
  // The allow dock command accepts a docking request.
    ...
    pub::SpaceObj::GetLocation(iShip, clients[iTargetClientID].vCarrierLocation,
...
void __stdcall PlayerLaunch(unsigned int iShip, unsigned int client) {
  ...
  if (clients[client].mobile_docked) {
    // Update the location of the carrier and remove the docked ship from the
    // carrier
    uint carrier_client =
...
EXPORT PLUGIN_INFO *Get_PluginInfo() {
  ...
  p_PI->lstHooks.push_back(PLUGIN_HOOKINFO(
      (FARPROC *)&CharacterInfoReq, PLUGIN_HkIServerImpl_CharacterInfoReq, 0));
  p_PI->lstHooks.push_back(
  ...
  p_PI->lstHooks.push_back(PLUGIN_HOOKINFO(
      (FARPROC *)&ReqRemoveItem_AFTER, PLUGIN_HkIServerImpl_ReqRemoveItem, 15));
