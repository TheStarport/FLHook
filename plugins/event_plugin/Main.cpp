        ...
        while (ini.read_value()) {
          if (ini.is_value("debug")) {
            ...
            set_mapCargoMissions.insert(
                std::multimap<uint, CARGO_MISSION>::value_type(mis.base, mis));
          } else if (ini.is_value("npc")) {
            NPC_MISSION mis;
            mis.nickname = ini.get_value_string(0);
            mis.system = CreateID(ini.get_value_string(1));
            mis.sector = ini.get_value_string(2);
            pub::Reputation::GetReputationGroup(mis.reputation,
                                                ini.get_value_string(3));
            mis.required_amount = ini.get_value_int(4);
            set_mapNpcMissions.insert(
                std::multimap<uint, NPC_MISSION>::value_type(mis.reputation,
                                                             mis));
          }
        }
      }
    ...
    for (; start != end; ++start) {
      if (start->second.system == iSystem) {
        ...
        if (start->second.curr_amount < start->second.required_amount) {
          ...
          // PrintUserCmdText(iClientID, L"%d ships remaining to destroy to
          // complete mission objective", needed);
        }
      }
    }
  }
...
EXPORT PLUGIN_INFO *Get_PluginInfo() {
  ...
  p_PI->lstHooks.push_back(PLUGIN_HOOKINFO((FARPROC *)&GFGoodSell,
                                           PLUGIN_HkIServerImpl_GFGoodSell, 0));
  return p_PI;
}
