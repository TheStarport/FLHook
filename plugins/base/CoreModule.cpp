...
CoreModule::CoreModule(PlayerBase* the_base)
    : Module(TYPE_CORE), base(the_base), space_obj(0), dont_eat(false), dont_rust(false)
{
}
...
void CoreModule::Spawn()
{
	if (!space_obj)
	{
		...
		if (set_plugin_debug > 1)
			Console::ConInfo(L"CoreModule::created space_obj=%u health=%f", space_obj, base->base_health);
...
bool CoreModule::Timer(uint time)
{
	...
	if (space_obj)
	{
		...
		if (!dont_eat)
		{
			...
			if (time % 28800 == 0)
			{
				for (auto i = set_base_crew_consumption_items.begin(); i != set_base_crew_consumption_items.end(); ++i)
				{
				...
				for (auto i = set_base_crew_food_items.begin(); i != set_base_crew_food_items.end(); ++i)
				{
					if (!crew_to_feed)
						break;
		...
		if (set_plugin_debug > 1)
			Console::ConInfo(L"CoreModule::timer space_obj=%u health=%f", space_obj, base->base_health);
...
void CoreModule::SetReputation(int player_rep, float attitude)
{
	if (space_obj)
	{
		...
		if (set_plugin_debug > 1)
			Console::ConInfo(
			    L"CoreModule::SetReputation player_rep=%u obj_rep=%u "
