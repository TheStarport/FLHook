...
namespace PlayerCommands
{
	...
	void BaseLogin(ClientId client, const std::wstring& args)
	{
		...
		PrintUserCmdText(
		    client,
		    L"Welcome administrator, all base command and "
	...
	void BaseDefenseMode(ClientId client, const std::wstring& args)
	{
		...
		else
		{
			...
			PrintUserCmdText(
			    client,
			    L"|  <mode> = 2 - neutral to non-allied "
	...
	void BaseFacMod(ClientId client, const std::wstring& args)
	{
		...
		else
		{
			...
			PrintUserCmdText(
			    client,
			    L"|  clear <index> - clear build queue for "
			    L"factory module at <index>");
			PrintUserCmdText(
			    client,
			    L"|  add <index> <type> - add item <type> to "
	...
	void BaseDefMod(ClientId client, const std::wstring& args)
	{
				...
				if (base->modules[index])
				{
					    ...
					    base->modules[index]->type == Module::TYPE_DEFENSE_3)
					{
						...
						PrintUserCmdText(
						    ...
						    index, mod->pos.x, mod->pos.y, mod->pos.z, mod->rot.z, mod->rot.y, mod->rot.z);
					}
				}
			}
			PrintUserCmdText(client, L"OK");
		...
		else
		{
			...
			PrintUserCmdText(
			    client,
			    L"|  list - show position and orientations of "
	...
	void BaseShieldMod(ClientId client, const std::wstring& args)
	{
		...
		for (uint index = 0; index < base->modules.size(); index++)
		{
			if (base->modules[index] && base->modules[index]->type == Module::TYPE_SHIELDGEN)
			{
	...
	static void ShowShopStatus(ClientId client, PlayerBase* base, int page)
	{
		...
		for (std::map<UINT, MARKET_ITEM>::iterator i = base->market_items.begin(); i != base->market_items.end();
		     ++i, item++)
		{
			if (item < start_item)
				continue;
