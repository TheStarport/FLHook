...
PlayerBase::PlayerBase(const std::string& the_path)
    : path(the_path), base(0), money(0), base_health(0), base_level(0), defense_mode(0), proxy_base(0), affiliation(0),
...
void PlayerBase::Save()
{
	...
	if (file)
	{
		...
		for (std::map<UINT, MARKET_ITEM>::iterator i = market_items.begin(); i != market_items.end(); ++i)
		{
			fprintf(
			    file, "commodity = %u, %u, %f, %u, %u\n", i->first, i->second.quantity, i->second.price,
...
float PlayerBase::SpaceObjDamaged(uint space_obj, uint attacking_space_obj, float curr_hitpoints, float damage)
{
	...
	if (client)
	{
		...
		{
			bool is_ally = false;
			for (std::list<std::wstring>::iterator i = ally_tags.begin(); i != ally_tags.end(); ++i)
			{
				if (charname.find(*i) == 0)
				{
					is_ally = true;
	...
	if (!this->shield_active_time && shield_state == SHIELD_STATE_ONLINE)
	{
		...
		if (set_plugin_debug > 1)
			Console::ConInfo(L"PlayerBase::damaged shield active=%u", this->shield_active_time);
	}
