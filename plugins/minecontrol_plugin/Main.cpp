...
static std::string GetTrimParam(const std::string &scLine, uint iPos) {
  ...
  while (scOut.size() &&
         (scOut[scOut.size() - 1] == L' ' || scOut[scOut.size() - 1] == L'\t' ||
...
void CheckClientSetup(uint iClientID) {
  if (!mapClients[iClientID].bSetup) {
    ...
    if (set_iPluginDebug > 1) {
      ConPrint(L"NOTICE: iClientID=%d iRepGroupID=%u iShipID=%u lstCargo=",
    ...
    // equipment then record the bonus and the weapon types that can be used to
    // gather the ore.
...
EXPORT void LoadSettings() {
  ...
  // Commodity, Bonus, Affiliation, List of ships, equipment or cargo separated
  // by commas. The [FieldBonus] section has the following format: Field, Bonus,
      ...
      if (ini.is_header("PlayerBonus")) {
        while (ini.read_value()) {
          ...
          set_mmapPlayerBonus.insert(
              std::multimap<uint, PLAYER_BONUS>::value_type(pb.iLootID, pb));
          if (set_iPluginDebug) {
            ConPrint(L"NOTICE: mining player bonus %s(%u) %2.2f %s(%u)\n",
                     stows(GetTrimParam(scLine, 0)).c_str(), pb.iLootID,
                     pb.fBonus, stows(GetTrimParam(scLine, 2)).c_str(),
                     pb.iRep);
          }
        }
        ...
        while (ini.read_value()) {
          ...
          if (set_iPluginDebug) {
            ConPrint(
                L"NOTICE: zone bonus %s fBonus=%2.2f iReplacementLootID=%s(%u) "
                L"fRechargeRate=%0.0f fMaxReserve=%0.0f\n",
                stows(scZone).c_str(), fBonus, stows(scReplacementLoot).c_str(),
                iReplacementLootID, fRechargeRate, fMaxReserve);
          }
        }
      }
    }
    ...
    for (CmnAsteroid::CAsteroidField *cfield = csys->FindFirst(); cfield;
      ...
      try {
        ...
        if (cfield->near_field(vPos) && zone && zone->lootableZone) {
          ...
          // If a non-rock is being shot we won't have an associated mining
          // event so ignore this.
          ...
          // If no mining bonus entry for this commodity is found, flag as no
          // bonus
          ...
          // If this minable commodity was not hit by the right type of gun,
          // flag as no bonus
          else if (find(ammolst->second.begin(), ammolst->second.end(),
                        ci.iProjectileArchID) == ammolst->second.end()) {
            bNoMiningCombo = true;
            if (cd.iDebug)
              PrintUserCmdText(iClientID, L"* Wrong gun");
          }

          // If either no mining gun was used in the shot, or the character
          // isn't using a valid mining combo for this commodity, set bonus to
          // *0.5
          ...
          // If this ship is has another ship targetted then send the ore into
          // the cargo hold of the other ship.
          ...
          int iLootCount =
              (int)(fRand * set_fGenericFactor * fZoneBonus * fPlayerBonus *
          ...
          if (mapClients[iClientID].iDebug) {
            PrintUserCmdText(iClientID,
                             ...
                             fRand, set_fGenericFactor, fPlayerBonus,
                             fZoneBonus, iLootCount, iLootID, iCrateID,
          ...
          if (mapClients[iClientID].tmMineAsteroidSampleStart < time(0)) {
            ...
            if (average > 2.0f) {
              AddLog("NOTICE: high mining rate charname=%s rate=%0.1f/sec "
                     "location=%0.0f,%0.0f,%0.0f system=%08x zone=%08x",
                     wstos((const wchar_t *)Players.GetActiveCharacterName(
                               iClientID))
                         .c_str(),
                     average, vPos.x, vPos.y, vPos.z, zone->iSystemID,
                     zone->iZoneID);
            }
          ...
          if (iLootCount) {
            ...
            pub::Player::AddCargo(iSendToClientID, iLootID, iLootCount, 1.0,
                                  false);
          }
          return;
        }
