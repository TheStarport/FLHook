...
{
	ZONE_BONUS()
	    : fBonus(0.0f), iReplacementLootID(0), fRechargeRate(0), fCurrReserve(100000), fMaxReserve(50000), fMined(0)
	{
	}
...
struct CLIENT_DATA
{
	CLIENT_DATA() : bSetup(false), iDebug(0), iPendingMineAsteroidEvents(0), iMineAsteroidEvents(0) {}
...
void CheckClientSetup(uint iClientID)
{
	if (!mapClients[iClientID].bSetup)
	{
		...
		{
			Console::ConInfo(
			    L"NOTICE: iClientID=%d iRepGroupID=%u iShipID=%u lstCargo=", iClientID, iRepGroupID, iShipID);
		...
		for (auto& i : set_mmapPlayerBonus)
		{
			...
			if (fBonus > 0.0f)
			{
				...
				if (set_iPluginDebug > 1)
				{
					Console::ConInfo(L"NOTICE: iClientID=%d iLootID=%08x fBonus=%2.2f\n", iClientID, iLootID, fBonus);
				}
			}
		}
...
EXPORT void LoadSettings()
{
	...
	if (set_iPluginDebug)
		Console::ConInfo(L"NOTICE: generic_factor=%0.0f debug=%d", set_fGenericFactor, set_iPluginDebug);
		...
		while (ini.read_header())
		{
			if (ini.is_header("PlayerBonus"))
			{
				while (ini.read_value())
				{
					...
					{
						Console::ConErr(
						    L"ERROR: %s:%d: item '%s' not valid", stows(ini.get_file_name()).c_str(),
						    ini.get_line_num(), stows(GetTrimParam(scLine, 0)).c_str());
						continue;
					}
					...
					{
						Console::ConErr(
						    L"ERROR: %s:%d: bonus not valid", stows(ini.get_file_name()).c_str(), ini.get_line_num());
						continue;
					}
					...
					if (GetTrimParam(scLine, 2) != "*")
					{
						...
						{
							Console::ConErr(
							    L"ERROR: %s:%d: reputation not valid", stows(ini.get_file_name()).c_str(),
							    ini.get_line_num());
							continue;
						}
					}
					...
					while (scShipOrEquip.size())
					{
						...
						else if (Archetype::GetEquipment(iItemID))
						{
							...
							if (eq->get_class_type() == Archetype::GUN)
							{
								...
								if (gun->iProjectileArchID && gun->iProjectileArchID != 0xBAADF00D &&
						...
						else
						{
							Console::ConErr(
							    L"ERROR: %s:%d: item '%s' not valid", stows(ini.get_file_name()).c_str(),
							    ini.get_line_num(), stows(scShipOrEquip).c_str());
						}
					...
					{
						Console::ConInfo(
						    L"NOTICE: mining player bonus %s(%u) %2.2f "
						    L"%s(%u)\n",
						    stows(GetTrimParam(scLine, 0)).c_str(), pb.iLootID, pb.fBonus,
						    stows(GetTrimParam(scLine, 2)).c_str(), pb.iRep);
					}
				}
			}
			else if (ini.is_header("ZoneBonus"))
			{
				while (ini.read_value())
				{
					...
					{
						Console::ConErr(
						    L"ERROR: %s:%d: zone not valid", stows(ini.get_file_name()).c_str(), ini.get_line_num());
						continue;
					}
					...
					{
						Console::ConInfo(
						    L"NOTICE: zone bonus %s fBonus=%2.2f "
						    L"iReplacementLootID=%s(%u) "
						    L"fRechargeRate=%0.0f fMaxReserve=%0.0f\n",
						    stows(scZone).c_str(), fBonus, stows(scReplacementLoot).c_str(), iReplacementLootID,
						    fRechargeRate, fMaxReserve);
					}
				}
			}
		}
		...
		for (CmnAsteroid::CAsteroidField* cfield = csys->FindFirst(); cfield; cfield = csys->FindNext())
		{
			try
			{
				...
				if (cfield->near_field(vPos) && zone && zone->lootableZone)
				{
					...
					fZoneBonus *=
					    set_mapZoneBonus[zone->iZoneID].fCurrReserve / set_mapZoneBonus[zone->iZoneID].fMaxReserve;
					...
					// of gun, flag as no bonus
					    ...
					    find(ammolst->second.begin(), ammolst->second.end(), ci.iProjectileArchID) ==
					    ammolst->second.end())
					{
						bNoMiningCombo = true;
						if (cd.iDebug)
					...
					int iLootCount =
					    (int)(fRand * set_fGenericFactor * fZoneBonus * fPlayerBonus * zone->lootableZone->dynamic_loot_count2);
					...
					if (mapClients[iClientID].iDebug)
					{
						PrintUserCmdText(
						    ...
						    fRand, set_fGenericFactor, fPlayerBonus, fZoneBonus, iLootCount, iLootID, iCrateID,
...
/// client.
void __stdcall MineAsteroid(
    uint& iClientSystemID, class Vector const& vPos, uint& iCrateID, uint& iLootID, uint& iCount, uint& iClientID)
{
