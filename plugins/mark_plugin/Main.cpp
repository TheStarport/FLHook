...
void UserCmd_MarkObj(uint iClientID, const std::wstring &wscParam) {
  ...
  case 1:
    PrintUserCmdText(iClientID,
                     L"Error: You must have something targeted to mark it.");
    break;
  case 2:
    ...
    if (Mark[iClientID].vAutoMarkedObjs[i] == iObject) {
      if (i != Mark[iClientID].vAutoMarkedObjs.size() - 1) {
        Mark[iClientID].vAutoMarkedObjs[i] =
            Mark[iClientID]
                .vAutoMarkedObjs[Mark[iClientID].vAutoMarkedObjs.size() - 1];
      }
  ...
  try {
    ...
    while (pPD = Players.traverse_active(pPD)) {
      ...
      for (uint i = 0; i < Mark[iClientID].vAutoMarkedObjs.size(); i++) {
        ...
        if (HkDistance3D(VTargetPos, VClientPos) >
          ...
          if (i != Mark[iClientID].vAutoMarkedObjs.size() - 1) {
            Mark[iClientID].vAutoMarkedObjs[i] =
                Mark[iClientID]
                    .vAutoMarkedObjs[Mark[iClientID].vAutoMarkedObjs.size() -
                                     1];
            i--;
          }
          Mark[iClientID].vAutoMarkedObjs.pop_back();
      ...
      for (uint i = 0; i < Mark[iClientID].vDelayedAutoMarkedObjs.size(); i++) {
        ...
        Vector VTargetPos;
        Matrix MTargetOri;
        pub::SpaceObj::GetLocation(Mark[iClientID].vDelayedAutoMarkedObjs[i],
                                   VTargetPos, MTargetOri);
        if (!(HkDistance3D(VTargetPos, VClientPos) >
              Mark[iClientID].fAutoMarkRadius)) {
          pub::Player::MarkObj(iClientID,
                               Mark[iClientID].vDelayedAutoMarkedObjs[i], 1);
      ...
      for (uint i = 0; i < vMarkSpaceObjProc.size(); i++) {
        ...
        if (set_bFlakVersion) {
          ...
          if (iType != OBJ_CAPITAL &&
            ...
            Vector VTargetPos;
            Matrix MTargetOri;
            pub::SpaceObj::GetLocation(iMarkSpaceObjProcShip, VTargetPos,
                                       MTargetOri);
    ...
    if (tmTimeNow - mark->time > 50) {
      ...
      while (pPD = Players.traverse_active(pPD)) {
        ...
        if (Players[iClientID].iSystemID == iItemSystem) {
          pub::SpaceObj::GetLocation(Players[iClientID].iShipID, vPlayer,
                                     mTemp);
...
EXPORT void __stdcall JumpInComplete(unsigned int iSystemID,
  ...
  for (uint i = 0; i < Mark[iClientID].vDelayedSystemMarkedObjs.size(); i++) {
    ...
    if (iTargetSystem == iSystemID) {
      pub::Player::MarkObj(iClientID,
                           Mark[iClientID].vDelayedSystemMarkedObjs[i], 1);
...
EXPORT void UserCmd_Help(uint iClientID, const std::wstring &wscParam) {
  ...
  PrintUserCmdText(
      ...
      L"contacts and have an arrow on the side of the screen, as well as have "
  ...
  PrintUserCmdText(
      ...
      L"automatically in the\n  range specified whether on or off. If you want "
...
EXPORT bool UserCmd_Process(uint iClientID, const std::wstring &wscCmd) {
  ...
  for (uint i = 0; (i < sizeof(UserCmds) / sizeof(USERCMD)); i++) {
    if (wscCmdLower.find(ToLower(UserCmds[i].wszCmd)) == 0) {
      ...
      returncode = SKIPPLUGINS_NOFUNCTIONCALL; // we handled the command, return
                                               // immediatly
      return true;
    }
  }
