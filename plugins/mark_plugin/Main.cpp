...
void UserCmd_AutoMark(uint iClientID, const std::wstring& wscParam)
{
	...
	if (!Mark[iClientID].bMarkEverything)
	{
		...
		if (wscEnabled == L"on") // AutoMark is being enabled
		{
			...
			PrintUserCmdText(
			    iClientID, L"Automarking turned on within a %g KM radius", Mark[iClientID].fAutoMarkRadius / 1000);
		}
	...
	else
	{
		...
		if (wscEnabled == L"off") // AutoMark is being disabled
		{
			...
			if (wscRadius.length())
				PrintUserCmdText(
				    iClientID, L"Automarking turned off; radius changed to %g KMs",
		...
		if (Mark[iClientID].vAutoMarkedObjs[i] == iObject)
		{
			if (i != Mark[iClientID].vAutoMarkedObjs.size() - 1)
			{
				Mark[iClientID].vAutoMarkedObjs[i] =
				    Mark[iClientID].vAutoMarkedObjs[Mark[iClientID].vAutoMarkedObjs.size() - 1];
			}
		...
		if (Mark[iClientID].vDelayedSystemMarkedObjs[k] == iObject)
		{
			if (k != Mark[iClientID].vDelayedSystemMarkedObjs.size() - 1)
			{
				Mark[iClientID].vDelayedSystemMarkedObjs[k] =
				    Mark[iClientID].vDelayedSystemMarkedObjs[Mark[iClientID].vDelayedSystemMarkedObjs.size() - 1];
			}
				...
				if (pub::SpaceObj::ExistsAndAlive(Mark[iClientID].vDelayedAutoMarkedObjs[i]))
				{
					if (i != Mark[iClientID].vDelayedAutoMarkedObjs.size() - 1)
					{
						Mark[iClientID].vDelayedAutoMarkedObjs[i] =
						    Mark[iClientID].vDelayedAutoMarkedObjs[Mark[iClientID].vDelayedAutoMarkedObjs.size() - 1];
						i--;
					}
			...
			for (uint i = 0; i < vMarkSpaceObjProc.size(); i++)
			{
				...
				if (set_bFlakVersion)
				{
					...
					if (iType != OBJ_CAPITAL &&
					    ((Mark[iClientID].bMarkEverything && iType == OBJ_FIGHTER) /* || iType==OBJ_FREIGHTER*/))
					{
						...
						if (iSpaceObjSystem != iSystem ||
						    HkDistance3D(VTargetPos, VClientPos) > Mark[iClientID].fAutoMarkRadius)
						{
...
{
	EXPORT void __stdcall JumpInComplete(uint& iSystemID, uint& iShip)
	{
			...
			if (pub::SpaceObj::ExistsAndAlive(Mark[iClientID].vMarkedObjs[i]))
			{
				if (i != Mark[iClientID].vMarkedObjs.size() - 1)
				{
					Mark[iClientID].vMarkedObjs[i] =
					    Mark[iClientID].vMarkedObjs[Mark[iClientID].vMarkedObjs.size() - 1];
					i--;
				}
				Mark[iClientID].vMarkedObjs.pop_back();
				continue;
	...
	EXPORT void __stdcall DisConnect(uint& iClientID, enum EFLConnection& p2)
	{
...
extern "C" EXPORT void ExportPluginInfo(PluginInfo* pi)
{
	...
	pi->emplaceHook(HookedCall::IServerImpl__JumpInComplete, &HkIServerImpl::JumpInComplete);
	pi->emplaceHook(HookedCall::IServerImpl__LaunchComplete, &HkIServerImpl::LaunchComplete);
	pi->emplaceHook(HookedCall::IServerImpl__BaseEnter, &HkIServerImpl::BaseEnter);
	pi->emplaceHook(HookedCall::IServerImpl__Update, &HkIServerImpl::Update);
	pi->emplaceHook(HookedCall::IServerImpl__DisConnect, &HkIServerImpl::DisConnect);
