...
void LoadSettings() {
  ...
  set_iTargetBaseID =
      CreateID(IniGetS(scPluginCfgFile, "General", "TargetBase", "li06_05_base")
                   .c_str());
  set_iTargetSystemID = CreateID(
      IniGetS(scPluginCfgFile, "General", "TargetSystem", "li06").c_str());
  set_iRestrictedSystemID = CreateID(
      IniGetS(scPluginCfgFile, "General", "RestrictedSystem", "iw09").c_str());
...
bool UserCmd_Process(uint client, const std::wstring &cmd) {
  ...
  if (!cmd.compare(L"/conn")) {
    ...
    uint system = 0;
    pub::Player::GetSystem(client, system);
    if (system == set_iRestrictedSystemID || system == set_iTargetSystemID ||
