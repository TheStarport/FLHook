...
void LoadSettings()
{
	...
	std::string scPluginCfgFile = std::string(szCurDir) + "\\flhook_plugins\\misc_commands.cfg";
...
void Timer()
{
	...
	for (mapInfo_map_iter_t iter = mapInfo.begin(); iter != mapInfo.end(); iter++)
	{
		...
		{
			HKPLAYERINFO p;
			if (HkGetPlayerInfo((const wchar_t*)Players.GetActiveCharacterName(iter->first), p, false) == HKE_OK &&
			    p.iShip)
			{
...
void UserCmd_Pos(uint iClientID, const std::wstring& wscParam)
{
	...
	wchar_t buf[100];
	_snwprintf_s(
	    buf, sizeof(buf), L"Position %0.0f %0.0f %0.0f Orient %0.0f %0.0f %0.0f", pos.x, pos.y, pos.z, erot.x, erot.y,
	    erot.z);
...
USERCMD UserCmds[] = {
	{ L"/lights", UserCmd_Lights }, { L"/shields", UserCmd_Shields }, { L"/pos", UserCmd_Pos },
	{ L"/stuck", UserCmd_Stuck },   { L"/droprep", UserCmd_DropRep }, { L"/dice", UserCmd_Dice },
	{ L"/coin", UserCmd_Coin },
};
