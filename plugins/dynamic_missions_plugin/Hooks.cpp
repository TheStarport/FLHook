...
void __stdcall HkActTrigger(TRIGGER_ARG *trigger) {
  wstring msg = L"Mission trigger activated with hash id: " +
                stows(itos(trigger->iTriggerHash)) + L"\n";
  ConPrint(msg);
  HkMsgU(msg);
}
...
int __stdcall HkActDebugMsg(ACTION_DEBUGMSG_DATA *action_dbgMsg) {
  wstring msg =
      L"Mission trigger (" + stows(itos(action_dbgMsg->iTriggerHash)) +
      L") sent debug msg: " + stows(string(action_dbgMsg->szMessage)) + L"\n";
  ConPrint(msg);
  HkMsgU(msg);
...
bool HkSinglePlayer() { return true; }
...
EXPORT void LoadSettings() {
  ...
  // hook debug msg
  char *pAddressActDebugMsg =
      ((char *)GetModuleHandle("content.dll") + 0x115BC4);
  ...
  char *pAddressCreateSolar =
      ((char *)GetModuleHandle("content.dll") + 0x1134D4);
...
HK_ERROR HkTest(uint iTest) {
  ...
  iControllerID = pub::Controller::Create("Content.dll", "Mission_01a", &params,
                                          (pub::Controller::PRIORITY)2);
...
void CmdTest(CCmds *classptr, uint iTest) {
  ...
  // right check
  if (!(classptr->rights & RIGHT_SUPERADMIN)) {
...
EXPORT PLUGIN_INFO *Get_PluginInfo() {
  ...
  p_PI->lstHooks.push_back(PLUGIN_HOOKINFO((FARPROC *)&HkIEngine::CShip_init,
                                           PLUGIN_HkIEngine_CShip_init, 0));
  p_PI->lstHooks.push_back(PLUGIN_HOOKINFO((FARPROC *)&HkIEngine::CShip_destroy,
