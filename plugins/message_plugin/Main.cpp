...
void LoadSettings()
{
	...
	std::string scPluginCfgFile = std::string(szCurDir) + "\\flhook_plugins\\message.cfg";
	...
	set_wscCmdEchoStyle = stows(IniGetS(scPluginCfgFile, "General", "CmdEchoStyle", "0x00AA0090"));
	set_iStandardBannerTimeout = IniGetI(scPluginCfgFile, "General", "StandardBannerDelay", 5);
	set_iSpecialBannerTimeout = IniGetI(scPluginCfgFile, "General", "SpecialBannerDelay", 60);
	set_wscDisconnectSwearingInSpaceMsg = stows(
	    IniGetS(scPluginCfgFile, "General", "DisconnectSwearingInSpaceMsg", "%player has been kicked for swearing"));
	set_fDisconnectSwearingInSpaceRange =
	    IniGetF(scPluginCfgFile, "General", "DisconnectSwearingInSpaceRange", 5000.0f);
...
bool UserCmd_Process(uint& iClientID, const std::wstring& wscCmd)
{
	...
	if (set_bCmdEcho)
	{
		...
		{
			if (!(wscCmd == L"/l" || wscCmd == L"/local" || wscCmd == L"/s" || wscCmd == L"/system" ||
			      wscCmd == L"/g" || wscCmd == L"/group" || wscCmd == L"/t" || wscCmd == L"/target" ||
			      wscCmd == L"/r" || wscCmd == L"/reply" || wscCmd.find(L"//") == 0 ||
			      wscCmd.find(L"*") == (wscCmd.length() - 1)))
			{
				std::wstring wscXML = L"<TRA data=\"" + set_wscCmdEchoStyle + L"\" mask=\"-1\"/><TEXT>" +
...
extern "C" EXPORT void ExportPluginInfo(PluginInfo* pi)
{
	...
	mailCommunicator =
	    static_cast<MailCommunicator*>(PluginCommunicator::ImportPluginCommunicator(MailCommunicator::pluginName));
