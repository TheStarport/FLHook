...
void MailShow(const std::wstring& wscCharname, const std::string& scExtension, int iFirstMsg)
{
	...
	PrintUserCmdText(
	    iClientID, L"Viewing #%02d-#%02d of %02d messages", iFirstMsg, iLastMsg, MailCount(wscCharname, scExtension));
}
...
{ // NOLINT(clang-diagnostic-shadow-field)
	this->SendMail = [](std::wstring character, std::wstring msg) {
		MailSend(character, MSG_LOG, msg);
	};
}
...
{
	int iNumberUnreadMsgs = MailCountUnread((const wchar_t*)Players.GetActiveCharacterName(iClientID), MSG_LOG);
	int iNumberMsgs = MailCount((const wchar_t*)Players.GetActiveCharacterName(iClientID), MSG_LOG);
	if (iNumberMsgs == 0)
	{
	...
	MailShow((const wchar_t*)Players.GetActiveCharacterName(iClientID), MSG_LOG, iFirstMsg);
}
...
void UserCmd_MailDel(uint iClientID, const std::wstring& wscParam)
{
	...
	if (MailDel((const wchar_t*)Players.GetActiveCharacterName(iClientID), MSG_LOG, iMsg))
