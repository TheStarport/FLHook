...
float PlayerBase::SpaceObjDamaged(uint space_obj, uint attacking_space_obj,
  ...
  if (client) {
    ...
    // hostile. We do the ally search after checking to see if this player is on
    // the hostile list because allies don't normally shoot at bases and so this
    // is more efficient than searching the ally list first.
    if (hostile_tags.find(charname) == hostile_tags.end()) {
      bool is_ally = false;
