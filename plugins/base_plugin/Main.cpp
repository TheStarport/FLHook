  ...
  for (; base != player_bases.end(); base++) {
    if (base->second->system == system) {
      ...
      if (set_plugin_debug > 1)
        ConPrint(
            L"SyncReputationForClientShip:: ship=%u attitude=%f base=%08x\n",
...
void LoadSettingsActual() {
      ...
      if (ini.is_header("recipe")) {
        ...
        while (ini.read_value()) {
          if (ini.is_value("nickname")) {
            ...
            recipe.consumed_items[CreateID(ini.get_value_string(0))] =
                ini.get_value_int(1);
          }
        }
  ...
  std::string path =
      std::string(datapath) + "\\Accts\\MultiPlayer\\player_bases\\base_*.ini";
...
void HkTimerCheckKick() {
  ...
  if ((curr_time % 13) == 0 && set_status_path_html.size() > 0) {
    ...
    if (file) {
      ...
      fprintf(file, "<table width=\"90%%\" border=\"1\" cellspacing=\"0\" "
                    "cellpadding=\"2\">\n");
      ...
      while (iter != player_bases.end()) {
        ...
        fprintf(file, "<td class=\"column0\">%s</td>",
                base->shield_state == PlayerBase::SHIELD_STATE_ACTIVE ? "On"
                                                                      : "Off");
  ...
  if (client && response == PROCEED_DOCK) {
    ...
    if (pbase) {
      ...
      if (pbase->shield_active_time) {
        ...
        pub::Player::SendNNMessage(client,
...
void __stdcall CharacterSelect_AFTER(struct CHARACTER_ID const &cId,
  ...
  if (clients[client].player_base) {
    ...
    // If the player file indicates that the ship is in a base but this isn't
    // this base then dump the ship into space.
...
void __stdcall BaseEnter(uint base, uint client) {
  ...
  // If the last player base is set then we have not docked at a non player base
  // yet.
...
bool __stdcall LaunchPosHook(uint space_obj, struct CEqObj &p1, Vector &pos,
  ...
  if (player_launch_base) {
    ...
    if (set_plugin_debug)
      ConPrint(
          L"LaunchPosHook[1] space_obj=%u pos=%0.0f %0.0f %0.0f dock_mode=%u\n",
...
void __stdcall GFGoodSell(struct SGFGoodSellInfo const &gsi,
  ...
  if (base) {
    ...
    if (base->money < price) {
      PrintUserCmdText(client,
                       L"ERR: Base cannot accept goods, insufficient cash");
      clients[client].reverse_sell = true;
      return;
    }
    ...
    if ((item.quantity + count) > item.max_stock) {
      PrintUserCmdText(client,
                       L"ERR: Base cannot accept goods, stock limit reached");
      clients[client].reverse_sell = true;
      return;
    }
  ...
  if (player_base) {
    ...
    if (clients[client].reverse_sell) {
      ...
      for (auto &ci : clients[client].cargo) {
        if (ci.iID == iID) {
          Server.ReqAddItem(ci.iArchID, ci.hardpoint.value, count, ci.fStatus,
                            ci.bMounted, client);
          return;
        }
      }
...
void __stdcall ReqAddItem_AFTER(unsigned int good, char const *hardpoint,
  ...
  if (base) {
    ...
    // Update the player CRC so that the player is not kicked for 'ship related'
    // kick
    HkPlayerRecalculateCRC(client);
...
void __stdcall HkCb_AddDmgEntry(DamageList *dmg, unsigned short p1,
  ...
  if (iDmgToSpaceID && dmg->get_inflictor_id()) {
    ...
    if (i != spaceobj_modules.end()) {
      if (set_plugin_debug)
        ConPrint(L"HkCb_AddDmgEntry iDmgToSpaceID=%u get_inflictor_id=%u "
...
EXPORT PLUGIN_INFO *Get_PluginInfo() {
  ...
  p_PI->lstHooks.push_back(PLUGIN_HOOKINFO((FARPROC *)&BaseEnter,
                                           PLUGIN_HkIServerImpl_BaseEnter, 0));
  p_PI->lstHooks.push_back(
      PLUGIN_HOOKINFO((FARPROC *)&BaseExit, PLUGIN_HkIServerImpl_BaseExit, 0));
  p_PI->lstHooks.push_back(
  ...
  p_PI->lstHooks.push_back(PLUGIN_HOOKINFO((FARPROC *)&GFGoodBuy,
                                           PLUGIN_HkIServerImpl_GFGoodBuy, 15));
  p_PI->lstHooks.push_back(PLUGIN_HOOKINFO(
      (FARPROC *)&ReqAddItem, PLUGIN_HkIServerImpl_ReqAddItem, 15));
  p_PI->lstHooks.push_back(PLUGIN_HOOKINFO(
      (FARPROC *)&ReqAddItem_AFTER, PLUGIN_HkIServerImpl_ReqAddItem_AFTER, 15));
  p_PI->lstHooks.push_back(PLUGIN_HOOKINFO(
  ...
  p_PI->lstHooks.push_back(PLUGIN_HOOKINFO((FARPROC *)&CShip_destroy,
                                           PLUGIN_HkIEngine_CShip_destroy, 0));
  p_PI->lstHooks.push_back(
