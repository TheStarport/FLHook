...
void BaseHelp(uint client, const std::wstring &args) {
  ...
  std::wstring help =
      ...
      L"<TRA bold=\"true\"/><TEXT>/base addtag [tag], /base rmtag [tag], /base "
      ...
      L"bold=\"false\"/><PARA/>"
      L"<TEXT>Set or clear the faction that this base is affiliated with. When "
      ...
      L"<TEXT>Withdraw, deposit or check the status of the credits held by the "
      L"base's bank.</TEXT><PARA/><PARA/>"

      L"<TRA bold=\"true\"/><TEXT>/shop price [item] [price] [min stock] [max "
      L"stock]</TEXT><TRA bold=\"false\"/><PARA/>"
      L"<TEXT>Set the [price] of [item]. If the current stock is less than "
      L"[min stock]"
      L" then the item cannot be bought by docked ships. If the current stock "
      L"is more or equal"
      L" to [max stock] then the item cannot be sold to the base by docked "
      L"ships.</TEXT><PARA/><PARA/>"
      L"<TEXT>To prohibit selling to the base of an item by docked ships under "
      L"all conditions, set [max stock] to 0."
      L"To prohibit buying from the base of an item by docked ships under all "
...
void BaseInfo(uint client, const std::wstring &args) {
  ...
  if (iPara > 0 && iPara <= MAX_PARAGRAPHS && cmd == L"a") {
    ...
    PrintUserCmdText(client,
                     L"|  <paragraph> The paragraph number in the range 1-%d",
                     MAX_PARAGRAPHS);
...
void BaseDefenseMode(uint client, const std::wstring &args) {
  ...
  if (wscMode == L"0") {
    ...
    PrintUserCmdText(client, L"|  <mode> = 0 - neutral to non-allied ships, "
                             L"docking restricted to allied ships");
    PrintUserCmdText(client, L"|  <mode> = 1 - hostile to non-allied ships, "
                             L"docking restricted to allied ships");
    PrintUserCmdText(
        client,
        L"|  <mode> = 2 - neutral to non-allied ships, docking unrestricted");
  ...
  if (cmd == L"list") {
    ...
    for (uint index = 1; index < base->modules.size(); index++) {
      if (base->modules[index]) {
        ...
        PrintUserCmdText(client, L"%u: Empty - available for new module",
                         index);
      }
    }
    PrintUserCmdText(client, L"OK");
    ...
    PrintUserCmdText(client, L"|  construct <index> <type> - start building "
    ...
    PrintUserCmdText(
        client, L"|     <type> = 7 - hyperspace survey manufacturing factory");
    PrintUserCmdText(
        client, L"|     <type> = 8 - cloaking device manufacturing factory");
    PrintUserCmdText(client,
...
void BaseFacMod(uint client, const std::wstring &args) {
  ...
  if (cmd == L"list") {
    ...
    PrintUserCmdText(
        client,
        L"|  clear <index> - clear build queue for factory module at <index>");
...
void BaseDefMod(uint client, const std::wstring &args) {
  ...
  if (cmd == L"list") {
    ...
    for (uint index = 0; index < base->modules.size(); index++) {
      if (base->modules[index]) {
        if (base->modules[index]->type == Module::TYPE_DEFENSE_1 ||
          ...
          PrintUserCmdText(
              client,
              L"Module %u: Position %0.0f %0.0f %0.0f Orient %0.0f %0.0f %0.0f",
              index, mod->pos.x, mod->pos.y, mod->pos.z, mod->rot.z, mod->rot.y,
              mod->rot.z);
        }
      }
    }
    ...
    PrintUserCmdText(
        client, L"|  set - <index> <x> <y> <z> <rx> <ry> <rz> - set the "
...
void BaseDeploy(uint client, const std::wstring &args) {
  ...
  if (GetPlayerBase(
          CreateID(PlayerBase::CreateBaseNickname(wstos(basename)).c_str()))) {
  ...
  for (map<uint, uint>::iterator i = construction_items.begin();
    ...
    if (material_available == false) {
      PrintUserCmdText(
          client, L"ERR Construction failed due to insufficient raw material.");
