...
FactoryModule::FactoryModule(PlayerBase* the_base, uint the_type) : Module(the_type), base(the_base)
{
	active_recipe.nickname = 0;
}
...
void FactoryModule::SaveState(FILE* file)
{
	...
	for (std::list<uint>::iterator i = build_queue.begin(); i != build_queue.end(); ++i)
	{
...
bool FactoryModule::AddToQueue(uint equipment_type)
{
	...
	{
		if (equipment_type == 8 || equipment_type == 9 || equipment_type == 10)
		{
			build_queue.push_back(CreateID(RECIPE_NAMES[equipment_type]));
			return true;
		}
