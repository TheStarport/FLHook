...
void AdminCmd_Chase(CCmds* cmds, const std::wstring& wscCharname)
{
	...
	cmds->Print(L"Jump to system=%s x=%0.0f y=%0.0f z=%0.0f", targetPlyr.wscSystem.c_str(), pos.x, pos.y, pos.z);
...
void AdminCmd_Pull(CCmds* cmds, const std::wstring& wscCharname)
{
	...
	cmds->Print(L"Jump to system=%s x=%0.0f y=%0.0f z=%0.0f", adminPlyr.wscSystem.c_str(), pos.x, pos.y, pos.z);
...
bool ExecuteCommandString(CCmds* cmds, const std::wstring& wscCmd)
{
	if (IS_CMD("move"))
	{
		...
		AdminCmd_Move(cmds, cmds->ArgFloat(1), cmds->ArgFloat(2), cmds->ArgFloat(3));
		return true;
	}
	...
	{
		if (AdminCmd_Beam(cmds, cmds->ArgCharname(1), cmds->ArgStrToEnd(2)))
		{
			returncode = ReturnCode::SkipAll;
			return true;
		}
