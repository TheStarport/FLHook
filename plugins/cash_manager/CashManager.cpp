...
namespace Plugins::CashManager
{
	...
	void UserCmdWithdrawMoneyByPassword(ClientId& client, const std::wstring& param)
	{
		...
		const auto withdrawal = MultiplyUIntBySuffix(GetParam(param, ' ', 3));
	...
	void TransferMoney(ClientId client, const std::wstring& param, const Bank& bank)
	{
		...
		const auto amount = MultiplyUIntBySuffix(GetParam(param, ' ', 2));
	...
	void ShowBankInfo(ClientId& client, const Bank& bank, bool showPass)
	{
		PrintUserCmdText(client,
		    std::format(L"Your Bank Information:\n"
		                L"|    Identifier: {} \n"
		                L"|    Password: {}\n"
		                L"|    Credits: {}",
		        bank.identifier.empty() ? L"N/A" : bank.identifier,
		        showPass ? bank.bankPassword : L"*****",
		        ToMoneyStr(bank.cash)));
	...
	void UserCommandHandler(ClientId& client, const std::wstring& param)
	{
		...
		{
			if (const uint withdrawAmount = MultiplyUIntBySuffix(GetParam(param, ' ', 1)); withdrawAmount > 0)
			{
				const auto bank = Sql::GetOrCreateBank(acc);
		...
		{
			const uint depositAmount = MultiplyUIntBySuffix(GetParam(param, ' ', 1));
			const auto bank = Sql::GetOrCreateBank(acc);
	...
	const std::vector commands = {{CreateUserCommand(L"/bank", L"", UserCommandHandler,
