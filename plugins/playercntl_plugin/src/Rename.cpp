  ...
  if (set_bCharnameTags) {
    ...
    for (std::map<std::wstring, TAG_DATA>::iterator i =
      ...
      if (tag == i->second.tag &&
        ...
        SaveSettings();
        PrintUserCmdText(iClientID, L"OK Created rename password %s for tag %s",
...
void Timer() {
  ...
  while (pendingRenames.size()) {
    ...
    // Delete the character from the existing account, create a new character
    // with the same name in this account and then copy over it with the save
    // character file.
    try {
      if (!acc)
        throw "no acc";
      ...
      AddLog("NOTICE: User rename %s to %s (%s)", wstos(o.wscCharname).c_str(),
             wstos(o.wscNewCharname).c_str(),
             wstos(HkGetAccountID(acc)).c_str());
    } catch (char *err) {
      AddLog("ERROR: User rename failed (%s) from %s to %s (%s)", err,
             wstos(o.wscCharname).c_str(), wstos(o.wscNewCharname).c_str(),
             wstos(HkGetAccountID(acc)).c_str());
    }
  }
...
bool UserCmd_RenameMe(uint iClientID, const std::wstring &wscCmd,
    ...
    for (std::map<std::wstring, TAG_DATA>::iterator i =
      ...
      if (wscNewCharname.find(i->first) == 0 &&
        ...
        if (!wscPassword.length()) {
          PrintUserCmdText(
              iClientID,
              L"ERR Name starts with an owned tag. Password is required.");
          return true;
        } else if (wscPassword != i->second.master_password &&
                   wscPassword != i->second.rename_password) {
          PrintUserCmdText(
              iClientID,
              L"ERR Name starts with an owned tag. Password is wrong.");
          return true;
        }
  ...
  o.scDestFileTemp =
      scAcctPath + wstos(wscDir) + "\\" + wstos(wscSourceFile) + ".fl.renaming";
...
bool Rename::UserCmd_MoveChar(uint iClientID, const std::wstring &wscCmd,
  ...
  // Kick
  HkKickReason(o.wscDestinationCharname,
               L"Moving character, please wait 10 seconds before reconnecting");
  HkKickReason(o.wscMovingCharname,
               L"Moving character, please wait 10 seconds before reconnecting");
  return true;
}
...
void AdminCmd_ShowTags(CCmds *cmds) {
  ...
  uint curr_time = (uint)time(0);
  for (std::map<std::wstring, TAG_DATA>::iterator i = mapTagToPassword.begin();
       i != mapTagToPassword.end(); ++i) {
    int last_access = i->second.last_access;
    int days = (curr_time - last_access) / (24 * 3600);
    cmds->Print(L"tag=%s master_password=%s rename_password=%s last_access=%u "
