...
static void CheckTransferLog(uint iClientID) {
  ...
  try {
    ...
    while (fgets(buf, 1000, f) != NULL) {
      ...
      uint lHiByte;
      uint lLoByte;
      while (scValue.length() > 3 &&
             sscanf_s(scValue.c_str(), "%02X%02X", &lHiByte, &lLoByte) == 2) {
...
bool GiveCash::UserCmd_GiveCash(uint iClientID, const std::wstring &wscCmd,
  ...
  if (cash < set_iMinTransfer || cash < 0) {
    PrintUserCmdText(iClientID, L"ERR Transfer too small, minimum transfer " +
  ...
  if (targetClientId != -1 && !HkIsInCharSelectMenu(targetClientId)) {
    if (HkAntiCheat(targetClientId) != HKE_OK) {
      ...
      AddLog("NOTICE: Possible cheating when sending %s credits from %s (%s) "
             "to %s (%s)",
  ...
  if (targetClientId != -1) {
    if (ClientInfo[iClientID].iTradePartner ||
      ...
      AddLog("NOTICE: Trade window open when sending %s credits from %s (%s) "
             "to %s (%s) %u %u",
             wstos(ToMoneyStr(cash)).c_str(), wstos(wscCharname).c_str(),
             wstos(HkGetAccountID(HkGetAccountByCharname(wscCharname))).c_str(),
             wstos(wscTargetCharname).c_str(),
  ...
  if (HkAntiCheat(iClientID) != HKE_OK) {
    PrintUserCmdText(iClientID, L"ERR Transfer failed");
    AddLog("NOTICE: Possible cheating when sending %s credits from %s (%s) to "
           "%s (%s)",
           wstos(ToMoneyStr(cash)).c_str(), wstos(wscCharname).c_str(),
           wstos(HkGetAccountID(HkGetAccountByCharname(wscCharname))).c_str(),
           wstos(wscTargetCharname).c_str(),
  ...
  if ((err = HkGetCash(wscTargetCharname, iCurrCash)) != HKE_OK ||
    ...
    AddLog("ERROR: Cash transfer error when sending %s credits from %s (%s) to "
  ...
  std::wstring msg = L"You have received " + ToMoneyStr(cash) +
                     L" credits from " + ((bAnon) ? L"anonymous" : wscCharname);
  if (targetClientId != -1 && !HkIsInCharSelectMenu(targetClientId)) {
  ...
  AddLog(
      "NOTICE: Send %s credits from %s (%s) to %s (%s)",
      wstos(ToMoneyStr(cash)).c_str(), wstos(wscCharname).c_str(),
      wstos(HkGetAccountID(HkGetAccountByCharname(wscCharname))).c_str(),
      wstos(wscTargetCharname).c_str(),
      wstos(HkGetAccountID(HkGetAccountByCharname(wscTargetCharname))).c_str());
...
bool GiveCash::UserCmd_DrawCash(uint iClientID, const std::wstring &wscCmd,
  ...
  if (HkAntiCheat(iClientID) != HKE_OK) {
    PrintUserCmdText(iClientID, L"ERR Transfer failed");
    AddLog("NOTICE: Possible cheating when drawing %s credits from %s (%s) to "
           "%s (%s)",
  ...
  if (targetClientId != -1) {
    if (ClientInfo[iClientID].iTradePartner ||
        ClientInfo[targetClientId].iTradePartner) {
      PrintUserCmdText(iClientID, L"ERR Trade window open");
      AddLog("NOTICE: Trade window open when drawing %s credits from %s (%s) "
             "to %s (%s) %u %u",
             wstos(ToMoneyStr(cash)).c_str(), wstos(wscTargetCharname).c_str(),
             wstos(HkGetAccountID(HkGetAccountByCharname(wscTargetCharname)))
                 .c_str(),
  ...
  if ((err = HkGetCash(wscCharname, iCurrCash)) != HKE_OK ||
      iCurrCash != iExpectedCash) {
    AddLog("ERROR: Cash transfer error when drawing %s credits from %s (%s) to "
           "%s (%s) current %s credits expected %s credits ",
           wstos(ToMoneyStr(cash)).c_str(), wstos(wscTargetCharname).c_str(),
           wstos(HkGetAccountID(HkGetAccountByCharname(wscTargetCharname)))
               .c_str(),
  ...
  AddLog(
      "NOTICE: Draw %s credits from %s (%s) to %s (%s)",
      wstos(ToMoneyStr(cash)).c_str(), wstos(wscTargetCharname).c_str(),
      wstos(HkGetAccountID(HkGetAccountByCharname(wscTargetCharname))).c_str(),
      wstos(wscCharname).c_str(),
      wstos(HkGetAccountID(HkGetAccountByCharname(wscCharname))).c_str());
