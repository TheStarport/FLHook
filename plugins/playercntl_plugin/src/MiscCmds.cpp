...
void MiscCmds::LoadSettings(const std::string &scPluginCfgFile) {
  ...
  set_wscCoinMsg = stows(
      IniGetS(scPluginCfgFile, "General", "CoinMsg", "%player tossed %result"));

  set_iSmiteMusicID =
      CreateID(IniGetS(scPluginCfgFile, "General", "SmiteMusic", "music_danger")
                   .c_str());
}
...
void MiscCmds::Timer() {
  ...
  for (mapInfo_map_iter_t iter = mapInfo.begin(); iter != mapInfo.end();
    ...
    if (iter->second.bShieldsDown) {
      HKPLAYERINFO p;
      if (HkGetPlayerInfo(
              (const wchar_t *)Players.GetActiveCharacterName(iter->first), p,
              false) == HKE_OK &&
          p.iShip) {
...
bool MiscCmds::UserCmd_DropRep(uint iClientID, const std::wstring &wscCmd,
  ...
  PrintUserCmdText(iClientID,
                   L"OK Reputation dropped, logout for change to take effect.");
...
bool MiscCmds::UserCmd_SelfDestruct(uint iClientID, const std::wstring &wscCmd,
  ...
  if (wscParam == L"0000") {
    ...
    PrintUserCmdText(iClientID, L"WARNING! SELF DESTRUCT WILL COMPLETELY AND "
...
bool MiscCmds::UserCmd_Shields(uint iClientID, const std::wstring &wscCmd,
  ...
  PrintUserCmdText(iClientID, L"Shields %s",
                   mapInfo[iClientID].bShieldsDown ? L"Disabled" : L"Enabled");
  return true;
}
