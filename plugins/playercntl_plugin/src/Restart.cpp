...
bool Restart::UserCmd_Restart(uint iClientID, const std::wstring &wscCmd,
  ...
  if (!::PathFileExistsA(restart.scRestartFile.c_str())) {
    restart.scRestartFile = std::string(szCurDir) +
                            "\\flhook_plugins\\restart\\_" + wstos(wscFaction) +
                            ".fl";
    if (!PathFileExistsA(restart.scRestartFile.c_str())) {
  ...
  if (iRank == 0 || iRank > set_iMaxRank) {
    PrintUserCmdText(
        iClientID,
        L"ERR You must create a new char to restart. Your rank is too high");
    return true;
  }
  ...
  if (iCash > set_iMaxCash) {
    PrintUserCmdText(
        iClientID,
        L"ERR You must create a new char to restart. Your cash is too high");
    return true;
  }
...
void Timer() {
  while (pendingRestarts.size()) {
    ...
    try {
      ...
      AddLog("NOTICE: User restart %s for %s", restart.scRestartFile.c_str(),
