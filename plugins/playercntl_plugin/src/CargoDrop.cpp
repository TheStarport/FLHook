...
void CargoDrop::LoadSettings(const std::string &scPluginCfgFile) {
  ...
  set_fDisconnectingPlayersRange =
      IniGetF(scPluginCfgFile, "General", "DisconnectingPlayersRange", 5000.0f);
    ...
    if (ClientInfo[iClientID].tmF1Time ||
      ...
      if (!iter.second.bF1DisconnectProcessed) {
        ...
        if (set_bLootDisconnectingPlayers &&
          ...
          if (HkEnumCargo(wscCharname, lstCargo, iRemainingHoldSize) ==
              HKE_OK) {
            for (auto &item : lstCargo) {
              if (!item.bMounted && set_mapNoLootItems.find(item.iArchID) ==
                                        set_mapNoLootItems.end()) {
                HkRemoveCargo(wscCharname, item.iID, item.iCount);
                Server.MineAsteroid(iSystem, vLoc, set_iLootCrateID,
...
void CargoDrop::SendDeathMsg(const std::wstring &wscMsg, uint iSystem,
  ...
  if (iHullDrop > 0) {
    ...
    if (set_iPluginDebug)
      ConPrint(L"NOTICE: player control cargo drop in system %08x at %f,%f,%f "
               L"for ship size of iShipSizeEst=%d iHullDrop=%d\n",
               iSystem, myLocation.x, myLocation.y, myLocation.z, iShipSizeEst,
               iHullDrop);

    Server.MineAsteroid(iSystem, myLocation, set_iLootCrateID, set_iHullItem1ID,
                        iHullDrop, iClientIDVictim);
    Server.MineAsteroid(iSystem, myLocation, set_iLootCrateID, set_iHullItem2ID,
                        (int)(0.5 * (float)iHullDrop), iClientIDVictim);
  }
}
