...
int HkCb_C4800Hook(int *a1, int *a2, int *zone, double *a4, int a5, int a6) {
  __try {
    ...
    AddLog("ERROR: Exception/Crash suppression content.dll:0xC608D(zone=%08x)",
           (zone ? *zone : 0));
    LOG_EXCEPTION
    return 0;
  }
...
static double __cdecl HkCb_TimingSeconds(__int64 &ticks_delta) {
  ...
  if (seconds < 0 || seconds > 10.0) {
    AddLog("ERROR: Time delta invalid seconds=%f ticks_delta=%I64i", seconds,
           ticks_delta);
  ...
  try {
    if (!bPatchInstalled) {
      ...
      if (hModContentAC) {
        ...
        // in my case) and player taken off from nearest base of this archetype
        // (Manhattan) This is caused by multiple players dying in the same
        // planet death zone. Also 000c458f error arises when nearby stations
        // within a zone ) are reputed not coinciding with reputation on the
        // client-side.
        {
        ...
        // missing contains the from, to and cargo entries (amongst other
        // stuff). Original Bytes: 8B F8 8B 17 8B CF
        {
        ...
        ReadProcMem((char *)hModContentAC + 0x11C970, &fpCrashProc6F8B330Old,
                    4);
        ...
        fpCrashProc6F78DD0Old = PatchCallAddr(
            (char *)hModContentAC, 0x5ED4B, (char *)HkCb_CrashProc6F78DD0Naked);
        PatchCallAddr((char *)hModContentAC, 0xBD96A,
                      (char *)HkCb_CrashProc6F78DD0Naked);

        // Hook for crash at 0xC71AE (confirmed)
        fpCrashProc6F671A0Old = PatchCallAddr((char *)hModContentAC, 0xBDC80,
        ...
        // Patch the NPC persist distance in MP to 6.5km and patch the max spawn
        // distance to 6.5km
...
void CrashCatcher::Shutdown() {
  if (bPatchInstalled) {
    ...
    if (hModContentAC) {
      ...
      // WriteProcMem((char*)hModContentAC + 0x113470, &fpCrashProc1b113470Old,
      // 4);
