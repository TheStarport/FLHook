...
void SwitchSystem(uint iClientID, uint system, Vector pos, Matrix ornt) {
  ...
  // Force a launch to put the ship in the right location in the current system
  // so that when the change system command arrives (hopefully) a fraction of a
  // second later the ship will appear at the right location.
  ...
  for (auto &iter : mapSurvey) {
    ...
    if (iShip == 0) {
      ...
      if (sm.charging_on) {
        ...
        // Turn off the charging effect if the charging has failed due to lack
        // of fuel and skip to the next player.
        if (!sm.charging_on) {
          sm.curr_charge = 0;
        ...
        if (sm.curr_charge >= sm.arch.survey_complete_charge) {
          ...
          PrintUserCmdText(iClientID, L"Survey %0.0f%% complete",
    ...
    if (iShip == 0) {
      ...
      if (jd.jump_timer > 0) {
        ...
        // Execute the jump and do the pop sound
          ...
          SwitchSystem(iClientID, jd.iTargetSystem, jd.vTargetPosition,
                       mShipDir);

          // Find all ships within the jump field including the one with the
          // jump engine.
            ...
            while (pPD = Players.traverse_active(pPD)) {
              ...
              if (pPD->iOnlineID != iClientID && iSystemID2 == iSystemID &&
                ...
                vNewTargetPosition.x =
                    jd.vTargetPosition.x + (vPosition.x - vPosition2.x);
                vNewTargetPosition.y =
                    jd.vTargetPosition.y + (vPosition.y - vPosition2.y);
                vNewTargetPosition.z =
                    jd.vTargetPosition.z + (vPosition.z - vPosition2.z);
                pub::Audio::PlaySoundEffect(pPD->iOnlineID,
                                            CreateID("dsy_jumpdrive_activate"));
        ...
        // Wait until the ship is in the target system before turning off the
        // fuse by holding the timer.
      ...
      if (jd.charging_on) {
        ...
        if (jd.curr_charge >= jd.arch.can_jump_charge) {
          ...
          if (!jd.charging_complete) {
            ...
            pub::Player::SendNNMessage(
                iClientID,
                pub::GetNicknameId("nnv_jumpdrive_charging_complete"));
        ...
        if (jd.charge_status != expected_charge_status) {
          ...
          PrintUserCmdText(iClientID, L"Jump drive charge %0.0f%%",
          ...
          if (find(jd.active_charge_fuse.begin(), jd.active_charge_fuse.end(),
      ...
      if (tb.iCheckSystemOrBase == iSystem) {
        if (tb.iCheckZonesTimer > 0) {
          if ((tb.iCheckZonesTimer % 10) == 0)
            PrintUserCmdText(iClientID, L"Jump to next zone in %d seconds",
                             tb.iCheckZonesTimer);
          tb.iCheckZonesTimer--;
        } else if (tb.iCheckZonesTimer == 0) {
          if (tb.lstCheckZones.size()) {
            ...
            PrintUserCmdText(
                iClientID, L"Testing zone %s (%0.0f %0.0f %0.0f) iteration %d",
                stows(lz.zoneNick).c_str(), lz.pos.x, lz.pos.y, lz.pos.z,
                tb.iCheckTestedZones);
...
bool HyperJump::UserCmd_ChargeJumpDrive(uint iClientID,
  ...
  if (jd.charging_on) {
    ...
    pub::Player::SendNNMessage(iClientID,
                               pub::GetNicknameId("nnv_jumpdrive_charging"));
...
bool HyperJump::UserCmd_ActivateJumpDrive(uint iClientID,
  ...
  if (!jd.charging_complete) {
    PrintUserCmdText(iClientID, L"Jump drive not ready");
    pub::Player::SendNNMessage(iClientID,
                               pub::GetNicknameId("nnv_jumpdrive_not_ready"));
    return true;
  }
...
void HyperJump::PlayerLaunch(unsigned int iShip, unsigned int iClientID) {
  ...
  Vector pos = {Players[iClientID].vPosition.x, Players[iClientID].vPosition.y,
