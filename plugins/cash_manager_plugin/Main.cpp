...
void LoadSettings(const std::string &scPluginCfgFile) {
    ...
    set_bCheatDetection =
        IniGetB(scPluginCfgFile, "General", "CheatDetection", true);
    set_iBlockedSystem = CreateID(
        IniGetS(scPluginCfgFile, "General", "BlockedSystem", "").c_str());
}
...
USERCMD UserCmds[] = {
    ...
    {L"/sendcash", UserCmd_GiveCash,
     L"Usage: /givecash <charname> <cash> [anon]"},
    {L"/set cashcode", UserCmd_SetCashCode, L"Usage: /set cashcode <code>"},
    {L"/showcash", UserCmd_ShowCash, L"Usage: /showcash <charname> <code>"},
