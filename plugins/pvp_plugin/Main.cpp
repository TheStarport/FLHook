      ...
      if (count <= 1) {
        if (HkIsValidClientID(iContestantID)) {
          ...
          std::wstring wscWinner =
              (const wchar_t *)Players.GetActiveCharacterName(iContestantID);
  ...
  if (ffas.find(iSystemID) == ffas.end()) {
    ...
    while (pPD = Players.traverse_active(pPD)) {
      ...
      if (iClientID == iClientID2)
        ...
        std::wstring msg =
            wscCharname +
            L" has started a Free-For-All tournament. Cost to enter is " +
    ...
    if (ffas[iSystemID].contestants.size() > 0) {
      PrintUserCmdText(iClientID,
                       L"Challenge issued. Waiting for others to accept.");
...
bool UserCmd_AcceptFFA(uint iClientID, const std::wstring &wscCmd,
  ...
  if (ffas.find(iSystemID) == ffas.end()) {
    ...
    if (ffas[iSystemID].buyin > 0 && iCash < ffas[iSystemID].buyin) {
      PrintUserCmdText(iClientID,
                       L"You don't have enough credits to join this FFA.");
      return true;
    }
    ...
    if (ffas[iSystemID].contestants[iClientID].bAccepted == false) {
      ...
      PrintUserCmdText(
          ...
          std::to_wstring(ffas[iSystemID].buyin) +
              L" credits have been deducted from your Neural Net account.");
      std::wstring msg = charname + L" has joined the FFA. Pot is now at " +
...
bool UserCmd_Duel(uint iClientID, const std::wstring &wscCmd,
  ...
  if (iAmount <= 0) {
    PrintUserCmdText(
        iClientID,
        L"Must specify a cash amount. Usage: /duel <amount> e.g. /duel 5000");
    return true;
  }
...
bool UserCmd_AcceptDuel(uint iClientID, const std::wstring &wscCmd,
  ...
  PrintUserCmdText(iClientID, L"You have no duel requests. To challenge "
