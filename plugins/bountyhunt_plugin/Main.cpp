...
void PrintBountyHunts(uint iClientID)
{
	if (lstBountyHunt.begin() != lstBountyHunt.end())
	{
		...
		{
			PrintUserCmdText(
			    iClientID, L"Kill %s and earn %u credits (%u minutes left)", it.wscTarget.c_str(), it.iCash,
...
void UserCmd_BountyHunt(uint iClientID, const std::wstring& wscParam)
{
	...
	HkMsgU(
	    bh.wscInitiator + L" offers " + std::to_wstring(bh.iCash) + L" credits for killing " + bh.wscTarget + L" in " +
		...
		if (it.uiTargetID == uiClientID)
		{
			...
			else
			{
				...
				if (wscWinnerCharname.size() > 0)
				{
					...
					HkMsgU(
					    wscWinnerCharname + L" has killed " + it.wscTarget + L" and earned " +
