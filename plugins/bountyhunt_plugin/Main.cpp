...
void PrintBountyHunts(uint iClientID)
{
	if (lstBountyHunt.begin() != lstBountyHunt.end())
	{
		...
		{
			PrintUserCmdText(iClientID, L"Kill %s and earn %u credits (%u minutes left)", it.wscTarget.c_str(), it.iCash, (uint)((it.msEnd - GetTimeInMS()) / 60000));
		}
	}
}

bool UserCmd_BountyHunt(uint iClientID, const std::wstring& wscCmd, const std::wstring& wscParam, const wchar_t* usage)
{
	if (!set_bBhEnabled)
	{
		PRINT_DISABLED();
	...
	HkMsgU(bh.wscInitiator + L" offers " + std::to_wstring(bh.iCash) + L" credits for killing " + bh.wscTarget + L" in " + std::to_wstring(uiTime) + L" minutes.");
	return true;
}

bool UserCmd_BountyHuntId(uint iClientID, const std::wstring& wscCmd, const std::wstring& wscParam, const wchar_t* usage)
{
	if (!set_bBhEnabled)
	{
		PRINT_DISABLED();
...
USERCMD UserCmds[] =
{
	{ L"/bountyhunt", UserCmd_BountyHunt, L"Usage: /bountyhunt <playername> <credits> <time>"},
	{ L"/bountyhuntid", UserCmd_BountyHuntId, L"Usage: /bountyhuntid <id> <credits> <time>"},
};
...
void LoadSettings()
{
	...
	set_scCfgGeneralFile = std::string(szCurDir) + "\\flhook_plugins\\bountyhunt.cfg";
	set_bBhEnabled = IniGetB(set_scCfgGeneralFile, "General", "EnableBountyHunt", false);
	set_iLowLevelProtect = IniGetI(set_scCfgGeneralFile, "General", "LevelProtect", 0);
}
