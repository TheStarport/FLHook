...
static void WriteProcMem(void *pAddress, void *pMem, int iSize) {
    ...
    DWORD dwOld;
    BOOL res = VirtualProtectEx(hProc, pAddress, iSize, PAGE_EXECUTE_READWRITE,
                                &dwOld);
...
std::string IniGetS(std::string scFile, std::string scApp, std::string scKey,
                    std::string scDefault) {
    char szRet[2048];
...
std::string Trim(std::string scIn) {
    ...
    while (scIn.length() && (scIn[scIn.length() - 1] == L' ' ||
                             scIn[scIn.length() - 1] == '	' ||
                             scIn[scIn.length() - 1] == '\n' ||
...
BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved) {
    ...
    if (fdwReason == DLL_PROCESS_ATTACH && !bPatchInstalled) {
        ...
        std::string scServerCfgFile =
            std::string(szCurDir) + "\\acctpath_patch.cfg";
        set_scDataPath =
            Trim(IniGetS(scServerCfgFile, "General", "DataPath", ""));
        ...
        if (hModCommon && set_scDataPath.length()) {
            ...
            DWORD dwJmpOffset = (DWORD)((char *)HkCb_GetUserDataPath) -
                                (DWORD)hModCommon - 0xA2FA0 - 5;
