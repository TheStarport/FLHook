...
void SetDir() {
    ...
    //	if ( (!(unsigned __int8)sub_45A470() && byte_67354C || *(float
    //*)&dword_673550 >= (double)flt_612270)
    //        && sub_5792B0(23) )
...
bool __stdcall HkCb_HandleKeyCmd(int keycmd, int dunno) {
    ...
    // case 0xb7: // turn left
            {
                    if (bTurretMode)
                    {
                            ...
                            IObjInspectImpl *ship = (IObjInspectImpl
    *)GetPlayerShip();

                            IBehaviorManager *m =
    GetBehaviorManager(GetPlayerShip()); Vector v = { -0.4, 0, 0 };
    ...
    *)GetPlayerShip();
                            ...
                            IBehaviorManager *m =
    GetBehaviorManager(GetPlayerShip()); Vector v = { 0, 0, 0};
                            m->set_user_turning_input_state(false);
    ...
    *)GetPlayerShip();
                            ...
                            IBehaviorManager *m =
    GetBehaviorManager(GetPlayerShip()); Vector v = m->get_ship_up_direction();
                            AddLog(L"%f %f %f\n", v.x, v.y, v.z);
                            return true;
                    }
            }
...
static const wchar_t *GetCustomIDS(uint ids_number) {
    ...
    map<uint, wstring>::const_iterator iter =
        set_mapServerIDStrings.find(ids_number);
...
int __stdcall HkCb_GetIDSString(int rsrc, unsigned int ids_number, wchar_t *buf,
...
typedef int(__cdecl *_GetString)(char *rsrc, uint ids, wchar_t *buf,
                                 int buf_chars);
extern _GetString GetString = (_GetString)0x4347E0;

typedef int(__cdecl *_FormatXML)(const wchar_t *buf, int buf_chars,
                                 RenderDisplayList &rdl, int iDunno);
extern _FormatXML FormatXML = (_FormatXML)0x57DB50;

/// Return true if we override the default infocard
bool __stdcall HkCb_GetInfocard(unsigned int ids_number,
...
void PublicClientPatchInstall() {
...

    {
        ...
        unsigned char patch2[] = {
            0x9C, 0xA1, 0x44, 0x97, 0x67, 0x00, 0x83, 0xC0, 0xF8, 0x50,
            0xFF, 0x15, 0x70, 0x64, 0x5C, 0x00, 0x59, 0x91, 0x9D, 0x74,
            0x0D, 0xA0, 0xCA, 0xEC, 0x67, 0x00, 0x88, 0x81, 0xF9, 0x00,
            0x00, 0x00, 0xEB, 0x25, 0xB0, 0x00, 0x86, 0x81, 0xF9, 0x00,
            0x00, 0x00, 0xA2, 0xCA, 0xEC, 0x67, 0x00};
...

    {
        ...
        unsigned char patch5[] = {0xD9, 0x15, 0x70, 0x52, 0x67, 0x00, 0xD8,
                                  0x0D, 0x84, 0x90, 0x67, 0x00, 0xD9, 0x1D,
                                  0x7C, 0x90, 0x67, 0x00, 0xC3};
...

    {
        ...
        unsigned char patch6[] = {0xD9, 0x15, 0x7C, 0x52,
                                  0x67, 0x00, 0xEB, 0xDE};
    ...
    {
        BYTE patch[] = {
            0xB9, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xE1,
        }; // mov ecx HkCb_InitShipNaked, jmp ecx
        ...
        *iAddr = reinterpret_cast<DWORD>((void *)HkCb_InitShipNaked);
        ...
        BYTE patch1[] = {
            0xB9, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xE1,
        }; // mov ecx HkCb_SetCamNaked, jmp ecx
    ...
    // Patch get ids string
    {
        BYTE patch2[] = {
            0xB9, 0x00, 0x00, 0x00,
            0x00, 0xFF, 0xE1, 0x90}; // mov ecx HkCb_GetIDSStringNaked, jmp ecx
    ...
    // Patch get infocard
    {
        BYTE patch[] = {
            0xB9, 0x00, 0x00, 0x00,
            0x00, 0xFF, 0xE1}; // mov ecx HkCb_GetInfocardNaked, jmp ecx
        DWORD *iAddr = (DWORD *)((char *)&patch + 1);
    ...
    // Patch keynames
    {
        ...
        *((char **)0x614FE8) = patch8; // USER_MANEUVER_SLIDE_EVADE
        ...
        BYTE patch[] = {
            0xB9, 0x00, 0x00, 0x00,
            0x00, 0xFF, 0xE1}; // mov ecx HkCb_HandleKeyCmdNaked, jmp ecx
        DWORD *iAddr = (DWORD *)((char *)&patch + 1);
    ...
    0x90, 0x90, 0x90, 0x90, 0x90, 0x90 }; WriteProcMem((char*)0x1B01D0, patch,
    sizeof(patch));
    }

    {
            BYTE patch[] = { 0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90,
    0x90, 0x90 }; WriteProcMem((char*)0x1B01F0, patch, sizeof(patch));
    }

    {
            BYTE patch[] = { 0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90, 0x90,
    0x90, 0x90, 0x90, 0x90 }; WriteProcMem((char*)0x1B0210, patch,
    sizeof(patch));
    }
...
void PublicServerClientPatchInstall() {
    ...
    {
        // common.dll 053048 0000->E31F = include external equipment in cargo
        // size PART 1 ~adoxa common.dll 05330E 0000->E31F = include external
        // equipment in cargo size PART 2 ~adoxa
        ...
        // common.dll 0A9BA3 18->00 = include external equipment in cargo size
        // PART 3 ~adoxa common.dll 0AA904 18->00 = include external equipment
        // in cargo size PART 4 ~adoxa
        BYTE patch2[] = {0x00};
