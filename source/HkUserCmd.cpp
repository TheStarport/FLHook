...
void PrintUserCmdText(uint iClientID, std::wstring wscText, ...)
{
	...
	std::wstring wscXML = L"<TRA data=\"" + set_wscUserCmdStyle + L"\" mask=\"-1\"/><TEXT>" + XMLText(wszBuf) + L"</TEXT>";
...
//	IniWrite(scUserFile, "Settings", "DieMsgStyle", std::to_string(dieMsgStyle));
...
void UserCmd_Help(uint iClientID, const std::wstring &wscParam)
{
	...
	if(singleCommandHelp) {
		...
		CALL_PLUGINS_NORET(PLUGIN_UserCmd_Help,,(uint iClientID, const std::wstring &wscParam),(iClientID,wscParam));
	}
