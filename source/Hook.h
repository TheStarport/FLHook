...
EXPORT void AddExceptionInfoLog(SEHException* ex);
	...
	#define CATCH_HOOK(e)                                                                                \
		...
		{                                                                                                \
			e;                                                                                           \
			AddBothLog(true, L"STL Exception in %s on line %d: %s.", __FUNCTION__, __LINE__, ex.what()); \
			AddExceptionInfoLog(0);                                                                      \
		}                                                                                                \
		catch (...)                                                                                      \
		{                                                                                                \
			e;                                                                                           \
			AddBothLog(true, L"Exception in %s on line %d.", __FUNCTION__, __LINE__);                    \
			AddExceptionInfoLog();                                                                       \
		}
#else
	#define TRY_HOOK try
...
EXPORT HK_ERROR HkGetAccountDirName(std::variant<uint, std::wstring> player, std::wstring& wscDir);
EXPORT HK_ERROR HkGetCharFileName(std::variant<uint, std::wstring> player, std::wstring& wscFilename);
...
EXPORT HK_ERROR HkMsg(std::variant<uint, std::wstring> player, const std::wstring& wscMessage);
...
EXPORT HK_ERROR HkFMsg(std::variant<uint, std::wstring> player, const std::wstring& wscXML);
...
EXPORT void FormatSendChat(
    uint iToClientID, const std::wstring& wscSender, const std::wstring& wscText, const std::wstring& wscTextColor);
...
EXPORT HK_ERROR HkKickReason(std::variant<uint, std::wstring> player, const std::wstring& wscReason);
EXPORT HK_ERROR HkBan(std::variant<uint, std::wstring> player, bool bBan);
EXPORT HK_ERROR HkBeam(std::variant<uint, std::wstring> player, const std::wstring& wscBasename);
EXPORT HK_ERROR HkSaveChar(std::variant<uint, std::wstring> player);
EXPORT HK_ERROR
    HkEnumCargo(std::variant<uint, std::wstring> player, std::list<CARGO_INFO>& lstCargo, int& iRemainingHoldSize);
EXPORT HK_ERROR HkRemoveCargo(std::variant<uint, std::wstring> player, uint iID, int iCount);
EXPORT HK_ERROR HkAddCargo(std::variant<uint, std::wstring> player, uint iGoodID, int iCount, bool bMission);
EXPORT HK_ERROR
    HkAddCargo(std::variant<uint, std::wstring> player, const std::wstring& wscGood, int iCount, bool bMission);
EXPORT HK_ERROR HkRename(std::variant<uint, std::wstring> player, const std::wstring& wscNewCharname, bool bOnlyDelete);
...
EXPORT HK_ERROR HkGetReservedSlot(std::variant<uint, std::wstring> player, bool& bResult);
EXPORT HK_ERROR HkSetReservedSlot(std::variant<uint, std::wstring> player, bool bReservedSlot);
EXPORT void HkPlayerAutoBuy(uint iClientID, uint iBaseID);
EXPORT HK_ERROR HkResetRep(std::variant<uint, std::wstring> player);
EXPORT HK_ERROR HkGetGroupMembers(std::variant<uint, std::wstring> player, std::list<GROUP_MEMBER>& lstMembers);
EXPORT HK_ERROR HkSetRep(std::variant<uint, std::wstring> player, const std::wstring& wscRepGroup, float fValue);
EXPORT HK_ERROR HkGetRep(std::variant<uint, std::wstring> player, const std::wstring& wscRepGroup, float& fValue);
EXPORT HK_ERROR HkReadCharFile(std::variant<uint, std::wstring> player, std::list<std::wstring>& lstOutput);
EXPORT HK_ERROR HkWriteCharFile(std::variant<uint, std::wstring> player, std::wstring wscData);
...
EXPORT HK_ERROR HkAddEquip(std::variant<uint, std::wstring> player, uint iGoodID, const std::string& scHardpoint);
...
const wchar_t* ToLogString(const T& val)
{
	...
	if constexpr (
	    std::is_same_v<DeclType, int> || std::is_same_v<DeclType, uint> || std::is_same_v<DeclType, float> ||
	    std::is_same_v<DeclType, double>)
	{
...
EXPORT bool HkAddCheaterLog(std::variant<uint, std::wstring> player, const std::wstring& wscReason);
...
EXPORT HK_ERROR HkGetPlayerInfo(std::variant<uint, std::wstring> player, HKPLAYERINFO& pi, bool bAlsoCharmenu);
EXPORT std::list<HKPLAYERINFO> HkGetPlayers();
EXPORT HK_ERROR HkGetConnectionStats(uint iClientID, DPN_CONNECTION_INFO& ci);
EXPORT HK_ERROR HkSetAdmin(std::variant<uint, std::wstring> player, const std::wstring& wscRights);
EXPORT HK_ERROR HkGetAdmin(std::variant<uint, std::wstring> player, std::wstring& wscRights);
...
// HkFLIni
EXPORT HK_ERROR HkFLIniGet(std::variant<uint, std::wstring> player, const std::wstring& wscKey, std::wstring& wscRet);
EXPORT HK_ERROR
    HkFLIniWrite(std::variant<uint, std::wstring> player, const std::wstring& wscKey, std::wstring wscValue);
...
extern EXPORT void HkAddHelpEntry(
