  ...
  try {
    ...
    if (exception) {
          ...
          exception->NumberParameters == 3) // C++ exception
      {
        ...
        const char *szName = info && info->pCatchableTypeArray &&
        ...
        const char *szMessage =
            i != info->pCatchableTypeArray->nCatchableTypes ? obj->what() : "";
        // C++ exceptions are triggered by RaiseException in kernel32, so use
        // ebp to get the return address
        iOffset = 0;
        szModName[0] = 0;
        if (reg) {
        ...
        AddBothLog("Name=\"%s\" Message=\"%s\" Offset=%x Module=\"%s\"", szName,
                   szMessage, iOffset, strrchr(szModName, '\\') + 1);
      }
    ...
    if (reg) {
      AddBothLog(
          "eax=%x ebx=%x ecx=%x edx=%x edi=%x esi=%x ebp=%x eip=%x esp=%x",
          reg->Eax, reg->Ebx, reg->Ecx, reg->Edx, reg->Edi, reg->Esi, reg->Ebp,
          reg->Eip, reg->Esp);
    } else {
