...
struct __vcrt_ptd {
  ...
  int _CatchStateInParent; // Used to link together the catch funclet with the
                           // parent. During dispatch contains state associated
                           // with catch in the parent. During unwind represents
                           // the current unwind state that is resumed to during
                           // collided unwind and used to look for handlers of
                           // the throwing dtor.
