...
std::wstring FLHookConfig::File() { return L"FLHook.json"; }
...
#ifndef CORE_REFL
    #define CORE_REFL
REFL_AUTO(type(FLHookConfig::Debug), field(debugMode), field(logTraceLevel));

REFL_AUTO(type(FLHookConfig::General), field(antiDockKill), field(antiF1), field(changeCruiseDisruptorBehaviour), field(disableCharfileEncryption),
          field(disconnectDelay), field(disableNPCSpawns), field(localTime), field(maxGroupSize), field(persistGroup), field(torpMissileBaseDamageMultiplier),
          field(logPerformanceTimers), field(chatSuppressList), field(noPVPSystems), field(antiBaseIdle), field(antiCharMenuIdle));
...
REFL_AUTO(type(FLHookConfig::MsgStyle), field(msgEchoStyle), field(deathMsgStyle), field(deathMsgStyleSys), field(kickMsgPeriod), field(kickMsg),
          field(userCmdStyle), field(adminCmdStyle), field(deathMsgTextAdminKill), field(deathMsgTextPlayerKill), field(deathMsgTextSelfKill),
          field(deathMsgTextNPC), field(deathMsgTextSuicide));
...
REFL_AUTO(type(FLHookConfig), field(debug), field(general), field(plugins), field(messageQueue), field(userCommands), field(bans), field(chatConfig),
          field(callsign));
#endif
