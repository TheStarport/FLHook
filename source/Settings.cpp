...
void LoadSettings() {
  ...
  set_wscDeathMsgStyleSys =
      stows(IniGetS(set_scCfgFile, "Style", "DeathMsgStyleSys", "0x1919BD01"));
  ...
  set_MKM_wscStyle =
      stows(IniGetS(set_scCfgFile, "MultiKillMessages", "Style", "0x1919BD01"));
  ...
  // help
  HkAddHelpEntry(L"/set diemsg", L"<visibility>",
  ...
  HkAddHelpEntry(
      ...
      L"Ignores all messages from the character with the associated client ID "
  ...
  HkAddHelpEntry(
      ...
      L"<param> can take one of the following values:\tinfo - display current "
      ...
      L"countermeasures\n\treload - enable/disable autobuy for nanobots/shield "
  ...
  HkAddHelpEntry(L"/ids", L"",
                 L"Lists all characters with their respective client IDs.", L"",
                 get_bTrue);
  ...
  HkAddHelpEntry(L"/credits", L"", L"Displays FLHook's credits.", L"",
                 get_bTrue);
  HkAddHelpEntry(L"/help", L"[<command>]",
