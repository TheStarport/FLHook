...
bool get_bUserCmdSetDieMsg(uint iClientID)
{
	return set_bUserCmdSetDieMsg;
}
bool get_bUserCmdSetDieMsgSize(uint iClientID)
{
	return set_bUserCmdSetDieMsgSize;
}
bool get_bUserCmdSetChatFont(uint iClientID)
{
	return set_bUserCmdSetChatFont;
}
bool get_bUserCmdIgnore(uint iClientID)
{
	return set_bUserCmdIgnore;
}
bool get_bAutoBuy(uint iClientID)
{
	return set_bAutoBuy;
}
...
void LoadSettings()
{
	...
	set_wscDeathMsgTextPlayerKill = stows(
	    IniGetS(set_scCfgFile, "Style", "DeathMsgTextPlayerKill", "Death: %victim was killed by %killer (%type)"));
	set_wscDeathMsgTextSelfKill =
	    stows(IniGetS(set_scCfgFile, "Style", "DeathMsgTextSelfKill", "Death: %victim killed himself (%type)"));
	set_wscDeathMsgTextNPC =
	    stows(IniGetS(set_scCfgFile, "Style", "DeathMsgTextNPC", "Death: %victim was killed by an NPC"));
	set_wscDeathMsgTextSuicide =
	    stows(IniGetS(set_scCfgFile, "Style", "DeathMsgTextSuicide", "Death: %victim committed suicide"));
	set_wscDeathMsgTextAdminKill =
	    stows(IniGetS(set_scCfgFile, "Style", "DeathMsgTextAdminKill", "Death: %victim was killed by an admin"));

	set_wscKickMsg = stows(IniGetS(
	    set_scCfgFile, "Style", "KickMsg",
	...
	HkAddHelpEntry(
	    L"/set diemsgsize", L"<size>", L"Sets your death message's text size. Options: small, default.", L"",
	...
	HkAddHelpEntry(
	    L"/ignore", L"<charname> [<flags>]", L"Ignores all messages from the given character.",
