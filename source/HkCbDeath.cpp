      ...
      if (iClientID) { // a player was killed
        ...
        if (iClientIDKiller) {
          ...
          if (iClientID == iClientIDKiller) {
            ...
            wscMsg =
                ReplaceStr(set_wscDeathMsgTextSelfKill, L"%victim", wscVictim);
          } else {
            wscEvent += L" type=player";
            std::wstring wscKiller =
                (wchar_t *)Players.GetActiveCharacterName(iClientIDKiller);
            wscEvent += L" by=" + wscKiller;

            wscMsg = ReplaceStr(set_wscDeathMsgTextPlayerKill, L"%victim",
                                wscVictim);
          ...
          if ((set_MKM_bActivated) && (iClientID != iClientIDKiller)) {
            ...
            for (auto &msg : set_MKM_lstMessages) {
              if (msg.iKillsInARow ==
                  ClientInfo[iClientIDKiller].iKillsInARow) {
                std::wstring wscXMLMsg = L"<TRA data=\"" + set_MKM_wscStyle +
                                         L"\" mask=\"-1\"/> <TEXT>";
                wscXMLMsg +=
                    XMLText(ReplaceStr(msg.wscMessage, L"%player", wscKiller));
                wscXMLMsg += L"</TEXT>";

                char szBuf[0xFFFF];
                uint iRet;
                if (!HKHKSUCCESS(
                        HkFMsgEncodeXML(wscXMLMsg, szBuf, sizeof(szBuf), iRet)))
                  break;
                  ...
                  if ((iClientID == iClientIDKiller) ||
                      ((iSystemID == iClientSystemID) &&
                       (((ClientInfo[iClientID].dieMsg == DIEMSG_ALL) ||
                         (ClientInfo[iClientID].dieMsg == DIEMSG_SYSTEM)) ||
          ...
          wscEvent += L" type=npc";
          std::wstring wscMsg =
              ReplaceStr(set_wscDeathMsgTextNPC, L"%victim", wscVictim);
          wscMsg = ReplaceStr(wscMsg, L"%type", wscType);
          ...
          std::wstring wscMsg =
              ReplaceStr(set_wscDeathMsgTextSuicide, L"%victim", wscVictim);
          ...
          std::wstring wscMsg =
              ReplaceStr(set_wscDeathMsgTextAdminKill, L"%victim", wscVictim);
