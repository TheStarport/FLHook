...
HK_ERROR HkEnumCargo(const std::wstring &wscCharname,
  ...
  eq = eqLst->next;
  while (eq != eqLst) {
    CARGO_INFO ci = {eq->sID,      (int)eq->iCount, eq->iGoodID,  eq->fStatus,
                     eq->bMission, eq->bMounted,    eq->hardpoint};
...
HK_ERROR HkRemoveCargo(const std::wstring &wscCharname, uint iID, int iCount) {
  ...
  for (auto &cargo : lstCargo) {
    if ((cargo.iID == iID) && (cargo.iCount < iCount))
      iCount =
          cargo.iCount; // trying to remove more than actually there, thus fix
  }
  ...
  if (iBase) { // player docked on base
    ...
    /*		// fix "Ship or Equipment not sold on base" kick
                    if(!bCargoFound)
                    {
                            ...
                            // add to check-list which is being compared to the
       users equip-list when saving char EquipDesc ed; memset(&ed, 0,
       sizeof(ed)); ed.id = sEquipID; ed.count = iCount; ed.archid = iGoodID;
...
HK_ERROR HkRename(const std::wstring &wscCharname,
  ...
  if (!flc_decode(scTmpPath.c_str(),
                  scNewCharfilePath
                      .c_str())) { // file wasn't encoded, thus simply rename it
...
void HkPlayerAutoBuy(uint iClientID, uint iBaseID) {
  ...
  if (ClientInfo[iClientID].bAutoBuyCD || ClientInfo[iClientID].bAutoBuyCM ||
    ...
    // add mounted equip to a new list and eliminate double equipment(such as 2x
    // lancer etc)
  ...
  for (auto &buy : lstCart) {
    ...
    for (auto &available : bi->lstMarketMisc) {
      if (available.iArchID == buy.iArchID) {
        ...
        if (iBaseRep == -1)
          continue; // rep can't be determined yet(space object not created
                    // yet?)
    ...
    if (iCash < iCost)
      PrintUserCmdText(iClientID, L"Auto-Buy(%s): FAILED! Insufficient Credits",
      ...
      PrintUserCmdText(iClientID, L"Auto-Buy(%s): Bought %u unit(s), cost: %s$",
...
bool IsInRange(uint iClientID, float fDistance) {
  std::list<GROUP_MEMBER> lstMembers;
  HkGetGroupMembers((const wchar_t *)Players.GetActiveCharacterName(iClientID),
                    lstMembers);
...
HK_ERROR HKGetShipValue(const std::wstring &wscCharname, float &fValue) {
  ...
  for (const auto &line : lstCharFile) {
    ...
    if (wscKey == L"cargo" || wscKey == L"equip") {
      ...
      uint iGoodCount = ToUInt(
          Trim(line.substr(iFindComma + 1, line.find(L",", iFindComma + 1))));
