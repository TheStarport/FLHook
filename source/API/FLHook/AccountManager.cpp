...
//TODO: implicit int to uint conversions have to throw an error, we can't risk casting changing memory
...
void __fastcall AccountManager::LoadPlayerMData(MPlayerDataSaveStruct* mdata, void* edx, struct INI_Reader* ini)
{
	...
	*rumorsEnd = PrepareRumorVectorFunc(mdata->receivedRumors.end(), mdata->receivedRumors.end(), mdata->receivedRumors.begin());
	...
	{
		FLMapInsertShipFunc(&mdata->killedShips, shipKilled.second, shipKilled.first);
	}
	for (auto abortedMission : character.randomMissionsAborted)
	{
		FLMapInsertRMFunc(&mdata->killedShips, abortedMission.second, abortedMission.first);
	}
	for (auto completedMission : character.randomMissionsCompleted)
	{
		FLMapInsertRMFunc(&mdata->killedShips, completedMission.second, completedMission.first);
	}
	for (auto failedMission : character.randomMissionsFailed)
	{
		FLMapInsertRMFunc(&mdata->killedShips, failedMission.second, failedMission.first);
	}
