...
namespace IServerImplHook
{
    ...
    void __stdcall AcceptTrade(ClientId client, bool _genArg1)
    {
        ...
        if (const auto skip = CallPlugins(&Plugin::OnAcceptTrade, client, _genArg1); !skip)
        {
            CALL_SERVER_PREAMBLE { Server.AcceptTrade(client, _genArg1); }
            CALL_SERVER_POSTAMBLE(true, );
        }

        CallPlugins(&Plugin::OnAcceptTradeAfter, client, _genArg1);
    }
    ...
    void __stdcall SetTradeMoney(ClientId client, ulong _genArg1)
    {
        ...
        if (const auto skip = CallPlugins(&Plugin::OnSetTradeMoney, client, _genArg1); !skip)
        {
            CALL_SERVER_PREAMBLE { Server.SetTradeMoney(client, _genArg1); }
            CALL_SERVER_POSTAMBLE(true, );
        }

        CallPlugins(&Plugin::OnSetTradeMoneyAfter, client, _genArg1);
    }
    ...
    void __stdcall RequestTrade(uint _genArg1, uint _genArg2)
    {
        ...
        if (const auto skip = CallPlugins(&Plugin::OnRequestTrade, _genArg1, _genArg2); !skip)
        {
            CALL_SERVER_PREAMBLE { Server.RequestTrade(_genArg1, _genArg2); }
            CALL_SERVER_POSTAMBLE(true, );
        }

        CallPlugins(&Plugin::OnRequestTradeAfter, _genArg1, _genArg2);
    }
    ...
    void __stdcall TradeResponse(const unsigned char* _genArg1, int _genArg2, ClientId client)
    {
        ...
        if (const auto skip = CallPlugins(&Plugin::OnTradeResponse, client, _genArg1, _genArg2); !skip)
        {
            CALL_SERVER_PREAMBLE { Server.TradeResponse(_genArg1, _genArg2, client); }
            CALL_SERVER_POSTAMBLE(true, );
        }

        CallPlugins(&Plugin::OnTradeResponseAfter, client, _genArg1, _genArg2);
    }
