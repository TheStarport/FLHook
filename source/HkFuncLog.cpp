...
bool HkAddCheaterLog(const std::wstring &wscCharname,
  ...
  fprintf(f,
          ...
          "%s(%s)(%s) [%s %s]\n",
          stNow.tm_mon + 1, stNow.tm_mday, stNow.tm_year + 1900, stNow.tm_hour,
          stNow.tm_min, stNow.tm_sec, wstos(wscReason).c_str(),
...
bool HkAddKickLog(uint iClientID, std::wstring wscReason, ...) {
  ...
  fprintf(f, "%.2d/%.2d/%.4d %.2d:%.2d:%.2d Kick (%s): %s(%s)(%s)\n",
          stNow.tm_mon + 1, stNow.tm_mday, stNow.tm_year + 1900, stNow.tm_hour,
          stNow.tm_min, stNow.tm_sec, wstos(wszBuf).c_str(),
