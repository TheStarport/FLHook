    ...
    while (pPD = Players.traverse_active(pPD)) {
      ...
      if (set_iAntiCharMenuIdle) { // anti charmenu-idle check
        if (HkIsInCharSelectMenu(iClientID)) {
          ...
          else if ((time(0) - ClientInfo[iClientID].iCharMenuEnterTime) >=
    ...
    while (pPD = Players.traverse_active(pPD)) {
      ...
      if (ClientInfo[iClientID].tmF1Time &&
        ...
        ClientInfo[iClientID].tmF1Time = 0;
                 ...
                 (timeInMS() >= ClientInfo[iClientID].tmF1TimeDisconnect)) {
        ulong lArray[64] = {0};
        lArray[26] = iClientID;
        __asm
        {
  ...
  TRY_HOOK {
    while (1) {
      ...
      for (auto &ip : lstMyResolveIPs) {
        ...
        wchar_t hostbuf[255];
        GetNameInfoW(reinterpret_cast<const SOCKADDR *>(&addr), sizeof(addr),
                     hostbuf, std::size(hostbuf), nullptr, 0, 0);
    ...
    for (auto &ip : g_lstResolveIPsResult) {
      ...
      for (auto &ban : set_setBans) {
        if (Wildcard::wildcardfit(wstos(ban).c_str(),
          ...
          HkAddKickLog(ip.iClientID, L"IP/Hostname ban(%s matches %s)",
