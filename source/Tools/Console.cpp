...
void Console::Log(std::wstring &wStr, va_list args, void* addr) {
    ...
    if (RtlPcToFileHeader(addr, (void **)&hmod)) {
        ...
        if (GetModuleFileName(hmod, sz, MAX_PATH)) {
            std::string path = sz;
            scText = std::string("(") + wstos(GetTimeString(set_bLocalTime)) + path.substr(path.find_last_of("\\") + 1) + ") " + scText;
        }
    }

    WriteConsole(GetStdHandle(STD_OUTPUT_HANDLE), scText.c_str(), DWORD(scText.length()),
                 &iCharsWritten, nullptr);
