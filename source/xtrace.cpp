...
BOOL InitSymInfo(PCSTR lpszInitialSymbolPath, BOOL bSysPath) {
  ...
  // requiring the symbols be loaded. This is the fastest, most efficient way to
  // use the symbol handler.". SYMOPT_UNDNAME is excluded to do the undecoration
  // ourselves.
...
LPCTSTR GetFuncName() {
  ...
  for (ULONG n = 0; n < 2; n++) {
    bResult =
        StackWalk(IMAGE_FILE_MACHINE_I386, hProcess, hThread, &callStack, NULL,
