        ...
        if (clientID) { // a player was hit
            ...
            if (set_bChangeCruiseDisruptorBehaviour) {
                if (((dmgList->get_cause() == 6) || (dmgList->get_cause() == 0x15)) &&
                    ...
                    dmgList->set_cause(
                        static_cast<enum DamageCause>(0xC0)); // change to sth else, so
...
void __stdcall AddDamageEntry(DamageList *dmgList, unsigned short subObjID,
                                float hitPts, enum DamageEntry::SubObjFate fate) {
    ...
    if (dmgList->get_cause() == 0xC0) {
        ...
        bool unk2 = false;
        float unk;
        pub::SpaceObj::GetInvincible(ClientInfo[g_DmgTo].iShip, unk1, unk2,
                                     unk);
    ...
    TRY_HOOK {
        ...
        if (hitPts == 0 && subObjID == 1) {
            ...
            if (clientIDKiller &&
                type & (OBJ_DOCKING_RING | OBJ_STATION | OBJ_WEAPONS_PLATFORM))
...
void __stdcall NonGunWeaponHitsBaseBefore(char *ECX, char *p1,
                                               DamageList *dmg) {
    CSimple *simple;
