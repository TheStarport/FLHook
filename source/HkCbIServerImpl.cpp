...
#define ISERVER_LOGARG_V(a)                                                    \
  if (set_bDebug)                                                              \
    AddDebugLog("     " #a ": %f %f %f", (float)a.x, (float)a.y, (float)a.z);
...
#define CHECK_FOR_DISCONNECT                                                   \
  {                                                                            \
    if (ClientInfo[iClientID].bDisconnected) {                                 \
      AddLog("ERROR: Ignoring disconnected client in " __FUNCTION__ " id=%u",  \
             iClientID);                                                       \
      return;                                                                  \
    };                                                                         \
  }
...
void __stdcall SubmitChat(struct CHAT_ID cId, unsigned long lP1,
  ...
  CALL_PLUGINS_V(PLUGIN_HkIServerImpl_SubmitChat, __stdcall,
                 (struct CHAT_ID cId, unsigned long lP1, void const *rdlReader,
                  struct CHAT_ID cIdTo, int iP2),
...
void __stdcall SPObjUpdate(struct SSPObjUpdateInfo const &ui,
  ...
  if (n > 1.21f || n < 0.81f) {
    AddLog(
        "ERROR: Non-normalized quaternion found in " __FUNCTION__ " for id=%u",
        iClientID);
    HkKick(Players[iClientID].Account);
    return;
  }
  ...
  if (abs(ui.vPos.x) > 1e7f || abs(ui.vPos.y) > 1e7f || abs(ui.vPos.z) > 1e7f) {
    AddLog("ERROR: Ship position out of bounds in " __FUNCTION__ " for id=%u",
           iClientID);
    HkKick(Players[iClientID].Account);
    return;
  }
...
void __stdcall SPObjCollision(struct SSPObjCollisionInfo const &ci,
  ...
  CALL_PLUGINS_V(PLUGIN_HkIServerImpl_SPObjCollision, __stdcall,
                 (struct SSPObjCollisionInfo const &ci, unsigned int iClientID),
                 (ci, iClientID));

  EXECUTE_SERVER_CALL(Server.SPObjCollision(ci, iClientID));

  CALL_PLUGINS_V(PLUGIN_HkIServerImpl_SPObjCollision_AFTER, __stdcall,
                 (struct SSPObjCollisionInfo const &ci, unsigned int iClientID),
                 (ci, iClientID));
}
...
void __stdcall LaunchComplete(unsigned int iBaseID, unsigned int iShip) {
  ...
  CALL_PLUGINS_V(PLUGIN_HkIServerImpl_LaunchComplete, __stdcall,
                 (unsigned int iBaseID, unsigned int iShip), (iBaseID, iShip));
  ...
  TRY_HOOK {
    ...
    if (wscCharBefore.compare(wscCharname) != 0) {
      ...
      if (set_bUserCmdHelp)
        PrintUserCmdText(
            iClientID,
            L"To get a list of available commands, type \"/help\" in chat.");
      ...
      for (auto &cargo : lstCargo) {
        if (cargo.iCount < 0) {
          HkAddCheaterLog(
              wscCharname,
              L"Negative good-count, likely to have cheated in the past");
...
void __stdcall OnConnect(unsigned int iClientID) {
  ...
  TRY_HOOK {
    // If ID is too high due to disconnect buffer time then manually drop the
    // connection.
    if (iClientID > MAX_CLIENT_ID) {
      AddLog(
          "INFO: Blocking connect in " __FUNCTION__ " due to invalid id, id=%u",
          iClientID);
...
void __stdcall DisConnect(unsigned int iClientID, enum EFLConnection p2) {
  ...
  CATCH_HOOK({
    AddLog("ERROR: Exception in " __FUNCTION__ "@loc2 charname=%s iClientID=%u",
...
void __stdcall TerminateTrade(unsigned int iClientID, int iAccepted) {
  ...
  TRY_HOOK {
    if (iAccepted) { // save both chars to prevent cheating in case of server
                     // crash
...
void __stdcall ActivateThrusters(unsigned int iClientID,
  ...
  CALL_PLUGINS_V(PLUGIN_HkIServerImpl_ActivateThrusters, __stdcall,
                 (unsigned int iClientID, struct XActivateThrusters const &at),
                 (iClientID, at));

  EXECUTE_SERVER_CALL(Server.ActivateThrusters(iClientID, at));

  CALL_PLUGINS_V(PLUGIN_HkIServerImpl_ActivateThrusters_AFTER, __stdcall,
                 (unsigned int iClientID, struct XActivateThrusters const &at),
                 (iClientID, at));
}
    ...
    for (auto &cargo : lstCargo) {
      if (cargo.iArchID == gsi.iArchID) {
        ...
        if (abs(gsi.iCount) > cargo.iCount) {
          ...
          const wchar_t *wszCharname =
              (wchar_t *)Players.GetActiveCharacterName(iClientID);
          swprintf_s(wszBuf, L"Sold more good than possible item=%08x count=%u",
...
void __stdcall SystemSwitchOutComplete(unsigned int iShip,
  ...
  CALL_PLUGINS_V(PLUGIN_HkIServerImpl_SystemSwitchOutComplete_AFTER, __stdcall,
                 (unsigned int iShip, unsigned int iClientID),
                 (iShip, iClientID));
}
...
void __stdcall Login(struct SLoginInfo const &li, unsigned int iClientID) {
  ...
  TRY_HOOK {
    ...
    rip.wscHostname = L"";
    rip.iConnects =
        ClientInfo[iClientID]
            .iConnects; // security check so that wrong person doesnt get banned
...
void __stdcall ReqModifyItem(unsigned short p1, char const *p2, int p3,
  ...
  CALL_PLUGINS_V(PLUGIN_HkIServerImpl_ReqModifyItem, __stdcall,
                 (unsigned short p1, char const *p2, int p3, float p4, bool p5,
                  unsigned int iClientID),
