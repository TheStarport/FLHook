...
class DLL MailManager : public Singleton<MailManager>
{
  ...
  public:
	...
	/// <returns>An std::vector of MailItem in s returned in the event of success, otherwise an std::wstring of an err.</returns>
	cpp::result<std::vector<MailItem>, std::wstring> GetMail(const std::variant<uint, std::wstring_view>& character, const bool& ignoreRead, const int& page);
	...
	/// <returns>A mail item in the event of success, otherwise an std::wstring of err</returns>
	...
	/// <returns>An int64 listing the amount of mail deleted in the event of success, otherwise an std::wstring of an err</returns>
