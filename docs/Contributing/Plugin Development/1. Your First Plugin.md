...
{
	if (global->config->notifyUserOnBaseEntry == false)
	{
		return;
	}
...
Now we need to make this function be called whenever a player enters a base. We also need to add our new configurable value to the Reflectable macro

```cpp

// As its a field of config, we add it as such
REFL_AUTO(type(Config), field(overrideUserNumber). field(notifyUserOnBaseEntry));
